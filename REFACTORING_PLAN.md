...
### Phase 6(PR #132〜#137、2026-06-12 完了)

| 指標 | 前 | 後 |
| --- | --- | --- |
| CLI E2E テスト数 | 259 | 248 |
| cli_integration 実行時間(llvm-cov 計測下) | 258.7 秒 | 194.2 秒 |
| E2E のみ到達の core production 行 | 1,343 | 1,187 |
| `test_support.rs` 再エクスポート名 | 181 | 96 |
| `test_support.rs` 行数 | 380 | 294 |
| テスト総数(workspace) | 1,911 | 1,924 |
| テスト総行数(両 tests) | 55,063 | 54,165 |
| カバレッジ(Line / Region) | 88.99% / 87.04% | 88.99% / 87.04% |

- 層別カバレッジ計測(`scripts/coverage-layers.sh`)を新設し、E2E のみ到達の
  production 行を抽出してから移譲する方式で全 PR の検出力非低下を機械的に
  保証した(各 PR で covered → uncovered の production 行ゼロを確認)
- 付録 7.5 の分類台帳(153 テスト)に基づき、移譲 11 件・縮小 14 件を実施。
  縮小対象のうち assert が既に CLI 表現のみだった 4 件は E2E 側変更なしで
  ユニット側の同等検証のみ追加した。台帳で「残す」とした 114 件は不変
- 各移譲 PR は「ユニット追加(単独で green)」→「E2E 縮小」の 2 段コミット構成。
  移譲中に検出したカバレッジ欠落 1 件(rewrap plan の BindingConfigured 分岐)は
  専用ユニットテストで即時解消した
- ブリッジ最終棚卸しで参照ゼロの再エクスポート 77 名を一括削除し、
  目標(100 以下)を達成。online feature 境界テストが存在を固定する
  GitHub 検証系 3 名は維持
- テスト層の役割分担(E2E / 外部ユニット / 内部ユニット)の判断基準は
  コーディングルール文書として別途固定した

## 9. 全体完了基準の検証(2026-06-12 時点、全フェーズ完了)

| 基準 | 結果 |
| --- | --- |
| 機能・CLI 仕様・ワイヤーフォーマット・公開 API 不変 | 達成(全 PR でスナップショット・E2E・スキーマ適合テスト通過) |
| カバレッジがベースライン以上 | 達成(Line 88.88% → 88.99%、Region 86.82% → 87.04%) |
| テストコード総行数の削減 | 達成(55,774 → 54,165 行) |
| production 総行数の削減 | 未達(47,891 → 49,226 行。テスト登録行 + 関数分割による増) |
| テスト追加・移動時の編集箇所削減 | 達成(ヘルパー一本化、手動登録 118 → 8、ブリッジ 429 → 96) |
| clippy 警告ゼロ・fmt 違反ゼロ | 達成 |

テスト実行時間は core 独立ユニットバイナリが約 46 秒 → 約 4 秒、CLI E2E が
約 259 秒 → 約 194 秒に短縮(いずれも llvm-cov 計測下の比較)。

## 10. 残課題

- production 総行数の削減は未達。重複は解消済みのため、今後の削減は
  機能整理(デッドコード化した DTO の追跡削除など)に依存する
- E2E のみ到達の core production 行が 1,187 行残る(大半は app 層の
  オーケストレーション)。テスト追加時は層選択基準に従い、E2E ではなく
  ユニット層でドメイン分岐を埋めることで漸減させる
- `cli_api::test_support` 再エクスポートは 96 名で目標(100 以下)を達成。
  残る 96 名は共有ヘルパー・ルート crate テスト・feature 境界テストが
  構造的に必要とする分であり、これ以上の削減はテスト基盤の再設計を要する
