...
use crate::app::context::member::{
    resolve_key_owner_with_access, resolve_required_member_with_access,
};
...
use crate::app::key::trust_signer::{
...
use crate::app::keystore::open_local_keystore;
use crate::app::trust::store::{read_stored_trust_signer, LocalStateBinding, StoredTrustSigner};
...
pub fn list_keys_command(
    ...
    let Some(access) = KeystoreAccess::open_optional_from_home(options.resolve_base_dir()?)? else {
...
pub fn activate_key_command(
    ...
    let capabilities = KeyCommandCapabilities::open(options.resolve_base_dir()?.as_path())?;
...
where
    Resign: FnOnce(&MemberHandle) -> Result<ExecutionContext>,
{
    let capabilities = KeyCommandCapabilities::open(options.resolve_base_dir()?.as_path())?;
    let access = capabilities.keystore();
...
pub fn export_key_command(
    ...
    let access = open_local_keystore(options.resolve_base_dir()?.as_path())?;
    let member_handle = resolve_required_member_with_access(&access, member_handle)?;
...
pub fn validate_kid(
    ...
    let access = open_local_keystore(options.resolve_base_dir()?.as_path())?;
    resolve_active_kid(&access, &MemberHandle::try_from(member_handle)?, kid)?;
    Ok(())
}
...
pub fn export_private_key_command(
    ...
    let loaded = load_selected_private_key(options, member_handle, kid, &ssh_ctx)?;
    let encoded_key = encode_portable_private_key(&loaded, password, password_policy)?;
