...
use crate::app::key::manage::KeyCommandCapabilities;
use crate::app::trust::store::{
    plan_trust_store_resign_bound, PlannedTrustStoreResign, StoredTrustSigner,
};
...
pub(super) struct TrustSignerRemovalRequest<'a> {
    pub(super) options: &'a CommonCommandOptions,
    ...
    Resign: FnOnce(&MemberHandle) -> Result<ExecutionContext>,
{
    ...
    if &execution.member_handle != request.member_handle {
        let error =
            build_foreign_signing_identity_error(request.member_handle, &execution.member_handle);
        return Err(FailedHandover::Unobserved(error));
    }
    let planned = plan_trust_store_resign_bound(
        request.options,
        &execution,
        &capabilities.local_state_binding(),
    )
