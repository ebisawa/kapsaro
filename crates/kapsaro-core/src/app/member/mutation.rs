...
pub fn add_member(options: &CommonCommandOptions, filename: &Path, force: bool) -> Result<String> {
    ...
    let workspace_dir = open_workspace_directory(&workspace.root_path)?;
...
pub fn evaluate_member_removal(
    ...
    let scan = scan_artifacts_for_member(&workspace_dir, member_handle, options)?;
...
fn scan_artifacts_for_member(
    ...
    for artifact in listing.artifacts {
        match artifact_contains_member(&artifact, member_handle, options.allow_expired_key) {
            Ok(result) => {
