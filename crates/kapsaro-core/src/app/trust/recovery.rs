...
use crate::app::trust::list::TrustListCommand;
use crate::error::{
...
pub fn observe_trust_store_recovery_from_execution(
    execution: &ExecutionContext,
) -> TrustStoreRecoveryToken {
    TrustStoreRecoveryToken {
        observed: observe_execution_trust_store_document(execution),
    }
}

fn observe_execution_trust_store_document(
    execution: &ExecutionContext,
) -> Result<Option<RegularFileSnapshot>> {
    let file_name = get_trust_store_file_name(&execution.member_handle);
    let trust_dir = execution.opened_trust_directory()?.cloned();
...
pub fn build_trust_store_reset_plan_from_list_command(
    ...
    let (error, cause) = prepare_reset_error(error, is_interactive)?;
...
pub fn build_trust_store_reset_plan_from_execution(
    execution: &ExecutionContext,
    token: TrustStoreRecoveryToken,
    error: Error,
    is_interactive: bool,
    ...
    let trust_dir = execution.opened_trust_directory()?.cloned();
    let path = get_trust_store_file_path(home.path(), &execution.member_handle);
    build_resolved_reset_plan(
...
fn enforce_interactive_reset(error: Error, is_interactive: bool) -> Result<Error> {
    if is_interactive {
        Ok(error)
    } else {
        Err(build_non_interactive_reset_error(error))
    }
}
...
fn prepare_reset_error(
    ...
    enforce_interactive_reset(error, is_interactive).map(|error| (error, cause))
}
