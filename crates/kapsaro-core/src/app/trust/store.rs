...
use crate::io::trust::store::{
    load_trust_store, load_trust_store_at, save_trust_store_at,
    TrustStoreLoadResult as IoTrustStoreLoadResult,
};
    ...
    F: FnOnce(&mut TrustStoreProtected) -> Result<TrustStoreMutation<T>>,
{
    ...
    lock::with_locked_dir(trust_dir, |locked_trust_dir| {
        let loaded = load_trust_store_for_mutation_at(
            locked_trust_dir,
            path,
            keystore_root,
            owner_handle,
...
fn load_trust_store_for_mutation_at<D>(
    dir: &D,
    path: &Path,
    keystore_root: &Path,
    owner_handle: &str,
    ...
    D: DirectoryFd,
{
    ...
    match mode {
        TrustStoreMutationMode::ExistingRequired => {
            load_existing_trust_store_at(dir, path, base_dir, keystore_root, owner_handle)
        }
        TrustStoreMutationMode::CreateIfMissing => {
            load_or_build_trust_store_at_dir(dir, path, base_dir, keystore_root, owner_handle)
        }
    }
}
...
fn save_changed_trust_store_at<D>(
    dir: &D,
    path: &Path,
    ...
    D: DirectoryFd,
{
    ...
    save_signed_trust_store_at(dir, path, protected, signing)
}

fn save_signed_trust_store_at<D>(
    dir: &D,
    path: &Path,
    protected: &TrustStoreProtected,
    signing: &VerifiedSigningContext<'_>,
    ...
    D: DirectoryFd,
{
    ...
    save_trust_store_at(dir, path, &document)
}
...
fn load_existing_trust_store_at<D>(
    dir: &D,
    path: &Path,
    base_dir: &Path,
    keystore_root: &Path,
    ...
    D: DirectoryFd,
{
    ...
    let loaded = load_trust_store_at(dir, path, base_dir)
        .map_err(|e| build_invalid_trust_store_error(path, e))?
        .ok_or_else(|| build_trust_store_not_found_error(owner_handle))?;
    verify_loaded_trust_store(path, keystore_root, loaded)
}

fn load_or_build_trust_store_at_dir<D>(
    dir: &D,
    path: &Path,
    base_dir: &Path,
    keystore_root: &Path,
    ...
    D: DirectoryFd,
{
    ...
    match load_trust_store_at(dir, path, base_dir)
        .map_err(|e| build_invalid_trust_store_error(path, e))?
    {
        Some(loaded) => verify_loaded_trust_store(path, keystore_root, loaded),
        None => {
