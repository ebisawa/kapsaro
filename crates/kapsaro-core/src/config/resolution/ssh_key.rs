...
//! 2. Environment variable (KAPSARO_SSH_IDENTITY)
...
pub(crate) enum SshKeySource {
    ...
    Cli,
    /// Environment variable (KAPSARO_SSH_IDENTITY)
    Env,
    /// Global config (KAPSARO_HOME/config.toml)
    GlobalConfig,
...
/// 2. `KAPSARO_SSH_IDENTITY` environment variable
/// 3. Global config (`KAPSARO_HOME/config.toml`)
...
pub(crate) fn build_ssh_key_not_found_error(candidate: &SshKeyResolution) -> Error {
    if !candidate.exists {
        let source_str = match candidate.source {
            ...
            SshKeySource::Env => "KAPSARO_SSH_IDENTITY",
