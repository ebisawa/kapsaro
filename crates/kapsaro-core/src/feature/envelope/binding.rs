...
pub fn build_kv_entry_aad(sid: &Uuid, key: &str) -> Result<Aad> {
    let bytes = build_kv_entry_aad_bytes(context::AAD_KV_ENTRY_PAYLOAD_V1, sid, key)?;
    Ok(Aad::from(bytes))
}
...
pub fn build_kv_wrap_info(sid: &Uuid, kid: &str) -> Result<Info> {
    let bytes = build_wrap_info_bytes(context::HPKE_INFO_KV_WRAP_V1, sid, kid)?;
    Ok(Info::from(bytes))
}

pub fn build_file_wrap_info(sid: &Uuid, kid: &str) -> Result<Info> {
    let bytes = build_wrap_info_bytes(context::HPKE_INFO_FILE_WRAP_V1, sid, kid)?;
    Ok(Info::from(bytes))
}
...
pub fn build_file_content_key_info(sid: &Uuid) -> Result<Info> {
    let bytes = build_sid_context_bytes(context::HKDF_INFO_FILE_CONTENT_KEY_V1, sid)?;
    Ok(Info::from(bytes))
}

pub fn build_file_mac_key_info(sid: &Uuid) -> Result<Info> {
    let bytes = build_sid_context_bytes(context::HKDF_INFO_FILE_MAC_KEY_V1, sid)?;
    Ok(Info::from(bytes))
}

pub fn build_kv_cek_info(sid: &Uuid, key: &str, nonce: &str) -> Result<Info> {
    let bytes = build_kv_cek_info_bytes(context::HKDF_INFO_KV_CEK_V1, sid, key, nonce)?;
    Ok(Info::from(bytes))
}

pub fn build_kv_mac_key_info(sid: &Uuid) -> Result<Info> {
    let bytes = build_sid_context_bytes(context::HKDF_INFO_KV_MAC_KEY_V1, sid)?;
    Ok(Info::from(bytes))
}
