...
pub fn build_sign_message(ikm_salt_b64: &str) -> String {
    format!(
        "{}\n{}",
        context::SSHSIG_MESSAGE_PREFIX_PRIVATE_KEY_PROTECTION_V1,
        ikm_salt_b64
    )
}
...
fn derive_key_from_raw_signature(
    ...
    let info = Info::from_string(&format!(
        "{}:{}",
        context::HKDF_INFO_PRIVATE_KEY_SSHSIG_V1,
        kid
    ));
...
fn non_deterministic_signature_error() -> crate::Error {
    crate::Error::build_crypto_error("W_SSH_NONDETERMINISTIC: SSH signature is non-deterministic")
}
