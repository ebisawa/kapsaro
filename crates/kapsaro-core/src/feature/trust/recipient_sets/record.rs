...
//! Recipient-set record construction and judgment.
//! Validates canonical kid lists, hashes, timestamps, and active-member hints.
...
use crate::model::wire::context::HASH_DOMAIN_RECIPIENT_SET_V1;
    ...
    for kid in recipient_kids {
        ...
        if !seen.insert(kid.clone()) {
            return Err(Error::build_verification_error(
                ...
                format!("Duplicate kid '{}' in recipient set", kid),
            ));
        }
    }
...
pub fn compute_recipient_set_hash(recipient_kids: &[String]) -> Result<String> {
    let canonical = jcs::normalize(&RecipientSetHashPayload {
        domain: HASH_DOMAIN_RECIPIENT_SET_V1,
        recipient_kids,
    })?;
...
pub fn validate_recipient_set_record(record: &RecipientSetRecord) -> Result<()> {
    Uuid::parse_str(&record.sid).map_err(|e| {
        Error::build_verification_error(
            "E_RECIPIENT_SET_INVALID_SID".to_string(),
            format!("recipient_sets[].sid must be a UUID: {}", e),
        )
    })?;
    let normalized = normalize_recipient_kids(record.recipient_kids.clone())?;
    if normalized != record.recipient_kids {
        return Err(Error::build_verification_error(
            "E_RECIPIENT_SET_KIDS_NOT_CANONICAL".to_string(),
            format!(
    ...
    if expected_hash != record.recipient_set_hash {
        return Err(Error::build_verification_error(
            "E_RECIPIENT_SET_HASH_MISMATCH".to_string(),
            format!("recipient_set_hash mismatch for sid '{}'", record.sid),
        ));
    }
...
pub(super) fn validate_approved_at(approved_at: &str) -> Result<()> {
