...
//! - kv-enc: Starts with ":KAPSARO_KV 1"
//! - file-enc: JSON with "format": "kapsaro:format:file-enc@1"
//! - kv-plain: UTF-8 text, 2+ non-empty/non-comment lines, 60%+ KEY=VALUE format

use crate::format::kv::HEADER_LINE_V1;
...
pub enum InputFormat {
    /// kv-enc format (:KAPSARO_KV 1)
    KvEnc,

    /// file-enc format (JSON with kapsaro:format:file-enc@1)
    FileEnc,
...
///
/// Format: ":KAPSARO_KV 1" (colon prefix + space separator)
fn detect_kv_enc(content: &str) -> Result<Option<InputFormat>> {
    if content.lines().next() == Some(HEADER_LINE_V1) {
...
///
/// Format: JSON with `protected.format = "kapsaro:format:file-enc@1"`
