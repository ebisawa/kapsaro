...
pub mod context {
    ...
    pub const AAD_KV_ENTRY_PAYLOAD_V1: &str = "kapsaro:aad:kv:entry-payload@1";

    /// HPKE info discriminator for kv-enc WRAP.
    pub const HPKE_INFO_KV_WRAP_V1: &str = "kapsaro:hpke-info:kv:wrap@1";
    /// HPKE info discriminator for file-enc WRAP.
    pub const HPKE_INFO_FILE_WRAP_V1: &str = "kapsaro:hpke-info:file:wrap@1";

    /// HKDF info for `PrivateKey` encryption key derivation from SSH signature.
    pub const HKDF_INFO_PRIVATE_KEY_SSHSIG_V1: &str = "kapsaro:hkdf-info:private-key:sshsig@1";
    /// HKDF info for `PrivateKey` encryption key derivation from password.
    pub const HKDF_INFO_PRIVATE_KEY_PASSWORD_V1: &str = "kapsaro:hkdf-info:private-key:password@1";
    /// HKDF salt discriminator for file-enc artifact key schedule.
    pub const HKDF_SALT_FILE_V1: &str = "kapsaro:hkdf-salt:file@1";
    /// HKDF salt discriminator for kv-enc artifact key schedule.
    pub const HKDF_SALT_KV_V1: &str = "kapsaro:hkdf-salt:kv@1";
    /// HKDF info discriminator for file-enc payload content key derivation.
    pub const HKDF_INFO_FILE_CONTENT_KEY_V1: &str = "kapsaro:hkdf-info:file:content-key@1";
    /// HKDF info discriminator for file-enc key-possession MAC key derivation.
    pub const HKDF_INFO_FILE_MAC_KEY_V1: &str = "kapsaro:hkdf-info:file:mac-key@1";
    /// HKDF info discriminator for kv-enc entry CEK derivation.
    pub const HKDF_INFO_KV_CEK_V1: &str = "kapsaro:hkdf-info:kv:cek@1";
    /// HKDF info discriminator for kv-enc key-possession MAC key derivation.
    pub const HKDF_INFO_KV_MAC_KEY_V1: &str = "kapsaro:hkdf-info:kv:mac-key@1";

    /// MAC domain separator for artifact key-possession proof.
    pub const MAC_DOMAIN_KEY_POSSESSION_V1: &str = "kapsaro:mac:key-possession@1";
    /// Signature-input domain separator for signed artifacts.
    pub const SIG_DOMAIN_ARTIFACT_SIGNATURE_V1: &str = "kapsaro:sig:artifact-signature@1";
    ...
    pub const SSHSIG_MESSAGE_DETERMINISM_CHECK_V1: &[u8] = b"kapsaro:sshsig:determinism-check@1";

    /// Hash domain separator for recipient set approval records.
    pub const HASH_DOMAIN_RECIPIENT_SET_V1: &str = "kapsaro:hash:recipient-set@1";
}
