...
//! Resolves recipients through fixed workspace directories, then encrypts and signs one file.
...
use crate::service::trust::{
    ensure_workspace_members_match_snapshot, ArtifactRecipientTrustOutcome, RecipientTrustOutcome,
    ReviewedTrustStore, TrustCommandSession, TrustContext, WorkspaceMemberSnapshot,
    WriteRecipientTrustPlan, WriteTrustOptions,
};
...
impl EncryptFileCommand<'_> {
    ...
    pub fn ensure_current_after_confirmation(&self) -> Result<()> {
        ...
        self.trust_store.ensure_current(self.capabilities.trust())
    }
    ...
    pub fn options(&self) -> WriteTrustOptions {
        self.options
    }
}
...
pub fn resolve_encrypt_file_command<'a>(
    ...
    let trust_plan = WriteRecipientTrustPlan::load(
        &capabilities,
        options,
        Some(capabilities.key_context().inner().local_key_identity()),
    )?;
    ...
    ConfirmRecipientSet: FnMut(&ArtifactRecipientTrustOutcome, &str) -> Result<bool>,
{
    ...
    review_artifact_output_recipient_set(
        ArtifactOutputRecipientSetReviewInput {
            session: command.capabilities.trust(),
