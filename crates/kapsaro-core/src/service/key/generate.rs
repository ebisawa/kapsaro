...
use crate::service::key::timestamp::{ensure_expiry_not_reached, resolve_key_timestamps};
use crate::service::key::types::KeyGenerationResult;
...
impl KeyGenerationHome {
    ...
    pub fn fix(local_state: &LocalStateSession) -> Result<Self> {
        local_state.ensured_home().map(Self::from_opened)
    }
