...
use super::diagnostics::{self, DiagnosticBatch};
...
/// One-shot trust exceptions consumed by a single read evaluation.
#[derive(Debug)]
...
pub struct KnownKeyApprovalEvidence {
    verified_github: Option<super::online::VerifiedGitHubEvidence>,
...
impl ReadTrustExceptions {
    ...
    /// Accept exactly one non-member signer identity for this evaluation.
    pub fn accepting_non_member(mut self, member_handle: MemberHandle, kid: Kid) -> Self {
...
impl KnownKeyReviewCandidate {
    pub(super) fn from_public_key(public_key: &PublicKey) -> Result<Self> {
        Ok(Self {
...
impl KnownKeyApprovalEvidence {
    ...
    pub fn with_verified_github_account(
        mut self,
        evidence: super::online::VerifiedGitHubEvidence,
    ) -> Self {
    ...
    pub(crate) fn known_key_with_evidence_for_test(
        ...
        if let Some((id, login, fingerprint, matched_key_id)) = github {
            evidence = evidence.with_verified_github_account(
                super::online::VerifiedGitHubEvidence::for_test(
                    &candidate,
                    id,
                    login,
                    fingerprint,
