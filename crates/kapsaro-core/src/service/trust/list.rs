...
use crate::support::fs::relative::{open_optional_child_dir, DirectoryFd, OpenDir};
use crate::Result;
...
pub fn resolve_trust_list_command(
    ...
    let Some(home) = local_state.home().cloned() else {
        return build_absent_trust_list_command(local_state.base_dir(), owner);
    };
