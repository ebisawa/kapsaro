...
use crate::service::trust::store::{
    execute_trust_store_resign_with_session, TrustStoreResignOutcome,
};
...
pub fn resign_trust_store_command(session: &TrustCommandSession) -> Result<TrustStoreResignResult> {
    let outcome = execute_trust_store_resign_with_session(session)?;
    Ok(build_resign_result(session.owner().as_str(), outcome))
}
