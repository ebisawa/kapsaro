...
//! Owns the lock and the commit order of a local trust store update on behalf of
//! `store.rs` and `core.rs`: observe first, then take the exclusive lock and write.

use crate::feature::trust::signer_keys::{document_signer_kid, SignerKeySnapshot};
use crate::feature::trust::store_mutation::{
    build_empty_trust_store, build_trust_store_not_found_error, judge_trust_store_write,
    TrustStoreMutation, TrustStoreState,
};
...
use crate::service::trust::persistence::{
    save_trust_store_for_write_at, TrustStoreMutationMode, TrustStoreMutationOutcome,
    TrustStoreMutationTarget,
};
use crate::service::trust::signer_snapshot::load_signer_key_snapshot;
...
impl TrustStorePreparation {
    ...
    pub(crate) fn from_reviewed_snapshot(
        ...
        load_signer_key_snapshot(keystore, owner, signer_kid).map(|signer_keys| Self {
            snapshot,
            signer_keys,
    ...
    /// The loaded document is handed back so a caller that also has to verify
        ...
        D: DirectoryFd + LockTargetDirectory,
    {
        ...
        let signer_keys = load_signer_key_snapshot(keystore, owner, signer_kid.as_ref())?;
        let prepared = Self {
...
pub(crate) fn verify_trust_store_with_owner_keys(
    ...
    let signer_keys = load_signer_key_snapshot(keystore, owner, document_signer_kid(doc).as_ref())?;
    ...
    F: FnOnce(&mut TrustStoreProtected) -> Result<TrustStoreMutation<T>>,
{
    with_exclusive_trust_store_load(
        ...
        |locked_trust_dir, loaded| {
            ...
            match content {
                TrustStoreContentSnapshot::Moved => Ok(TrustStoreCommitAttempt::Stale(mutate)),
                TrustStoreContentSnapshot::Unusable(error) => {
                    Ok(TrustStoreCommitAttempt::Unusable(error))
                }
                TrustStoreContentSnapshot::Usable(state) => {
...
enum TrustStoreContentSnapshot {
...
) -> Result<TrustStoreContentSnapshot> {
    let loaded = match loaded {
        Ok(loaded) => loaded,
        Err(error) => return evaluate_locked_failure(error),
    };
    if TrustStoreSnapshot::from_loaded(loaded.as_ref()) != prepared.snapshot {
        return Ok(TrustStoreContentSnapshot::Moved);
    }
...
fn build_trust_store_for_commit(
    ...
    match (target.mode, loaded) {
        (_, Some(loaded)) => match verify_stored_trust_store(loaded, &prepared.signer_keys) {
            Ok(stored) => Ok(TrustStoreContentSnapshot::Usable(stored.into())),
            Err(error) => evaluate_locked_failure(error),
        },
        (TrustStoreMutationMode::CreateIfMissing, None) => {
            build_empty_trust_store(target.owner).map(TrustStoreContentSnapshot::Usable)
        }
...
/// travels as itself.
fn evaluate_locked_failure(error: Error) -> Result<TrustStoreContentSnapshot> {
    if error.kind().is_content_failure() {
        Ok(TrustStoreContentSnapshot::Unusable(error))
    } else {
        Err(error)
    }
}
...
#[cfg(test)]
#[path = "../../../tests/unit/internal/service_trust_transaction_test.rs"]
mod service_trust_transaction_test;
