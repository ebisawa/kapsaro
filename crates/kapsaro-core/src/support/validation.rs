...
pub fn validate_member_handle(id: &str) -> Result<()> {
    ...
    if id.len() > 254 {
        return Err(Error::build_invalid_argument_error(format!(
            "member_handle too long: {} chars (max 254)",
            id.len()
        )));
    }

    let first = id.chars().next().ok_or_else(|| {
        Error::build_invalid_argument_error("member_handle cannot be empty".to_string())
    })?;
...
pub fn validate_github_login(login: &str) -> Result<()> {
    ...
    if login.len() > 39 {
        return Err(Error::build_invalid_argument_error(format!(
            "GitHub login too long: {} chars (max 39)",
            login.len()
        )));
    }
