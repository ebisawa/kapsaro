...
fn test_hpke_info_file() {
    ...
    // Should have required fields
    assert_eq!(parsed["p"], wire_context::HPKE_INFO_FILE_WRAP_V1);
    assert_eq!(parsed["sid"], sid.to_string());
    assert_eq!(parsed["kid"], kid);
}
...
fn test_cek_info_kv() {
    ...
    assert_eq!(parsed["p"], wire_context::HKDF_INFO_KV_CEK_V1);
    assert_eq!(parsed["sid"], sid.to_string());
...
fn test_aad_payload_kv() {
    ...
    // Should have required fields
    assert_eq!(parsed["p"], wire_context::AAD_KV_ENTRY_PAYLOAD_V1);
    assert_eq!(parsed["sid"], sid.to_string());
    assert_eq!(parsed["k"], key);
...
fn test_aad_file_payload() {
    ...
    assert_eq!(parsed["format"], format::FILE_PAYLOAD_V1);
    assert_eq!(parsed["sid"], sid.to_string());
...
fn test_aad_private_key() {
    ...
    assert_eq!(parsed["format"], format::PRIVATE_KEY_V1);
