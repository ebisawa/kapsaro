...
fn rewrap_review_rejects_changed_post_promotion_members() {
    ...
    let error = match session.resume_file_rewrap(review, RewrapOptions::new(), None) {
...
fn non_member_rewrap_exposes_recipient_reviews_only_after_signer_acceptance() {
    ...
    let RewrapSessionDecision::ReviewRequired(recipient_review) = session
        .resume_file_rewrap(signer_review, RewrapOptions::new(), Some(acceptance))
        .unwrap()
    else {
    ...
    F: FnOnce(&RecipientKeys, &KeyContext) -> String,
{
    ...
    let authorized = if name.ends_with(".kvenc") {
        let RewrapSessionDecision::Authorized(authorized) = session
            .begin_kv_rewrap(target, RewrapOptions::new(), false)
            .unwrap()
        else {
            panic!("self-only explicit KV rewrap must be authorized");
        };
        authorized
    } else {
        let RewrapSessionDecision::Authorized(authorized) = session
            .begin_file_rewrap(target, RewrapOptions::new(), false)
            .unwrap()
        else {
...
fn test_rewrap_rejects_recipients_from_stale_post_promotion_snapshot() {
    ...
    let error = match post.evaluate_file_rewrap(
        &pre,
        verified,
        recipients,
        &key_ctx,
