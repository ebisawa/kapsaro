...
fn test_trust_snapshots_keep_self_trust_bound_to_loaded_keystore() {
    ...
    let active = find_active_key_document(ALICE_MEMBER_HANDLE, &keystore_root)
        .unwrap()
        .expect("expected active key fixture");
    let original_identity = build_signer_identity(&active.public_key).unwrap();
    ...
    let replacement_identity = build_signer_identity(&replacement).unwrap();
    let execution = build_test_execution_context(&dir, ALICE_MEMBER_HANDLE, Some(&workspace));
...
fn test_evaluate_signer_trust_with_proof_missing_signer_pub_fails() {
    let ctx = build_test_trust_ctx(StrictKeyChecking::Yes, true);
    // Built field by field: the constructor always carries an embedded signer
    // key, and this is the proof that arrives without one.
    let proof = SignatureVerificationProof {
        member_handle: "alice@example.com".to_string(),
        kid: "KID1AAAA1111BBBB2222CCCC3333DDDD".to_string(),
        signer_public_key: None,
        verifying_key_source: VerifyingKeySource::SignerPubEmbedded,
        warnings: Vec::new(),
    };
