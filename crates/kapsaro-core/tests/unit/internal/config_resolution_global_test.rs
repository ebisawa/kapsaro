...
fn test_resolve_config_value_global() {
    ...
    let value = resolve_config_value("member_handle", temp_dir.path()).unwrap();
...
fn test_resolve_workspace_config_value_global() {
    ...
    let value = resolve_config_value("workspace", temp_dir.path()).unwrap();
...
fn test_resolve_config_value_reads_an_accepted_alias() {
    ...
    let value = resolve_config_value("gihub_user", temp_dir.path()).unwrap();
...
fn test_resolve_config_value_rejects_an_unsupported_key() {
    ...
    let error = resolve_config_value("no_such_key", temp_dir.path()).unwrap_err();
