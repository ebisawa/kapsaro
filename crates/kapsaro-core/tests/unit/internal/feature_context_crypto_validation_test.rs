...
use crate::feature::key::material::{
    parse_okp_key, verify_ed25519_consistency, verify_x25519_consistency,
};
...
fn test_validate_okp_key_wrong_kty() {
    ...
    let result = parse_okp_key("RSA", "Ed25519", "Ed25519", &d, &x, "Sig");
...
fn test_validate_okp_key_wrong_crv() {
    let d = encode_base64url_nopad(&[0u8; 32]);
    let x = encode_base64url_nopad(&[1u8; 32]);
    let result = parse_okp_key("OKP", "P-256", "Ed25519", &d, &x, "Sig");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
...
fn test_validate_okp_key_wrong_d_length() {
    ...
    let x = encode_base64url_nopad(&[1u8; 32]);
    let result = parse_okp_key("OKP", "Ed25519", "Ed25519", &d, &x, "Sig");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
...
// ============================================================================
// verify_ed25519_consistency tests
// ============================================================================
...
fn test_validate_ed25519_consistency_mismatch() {
    ...
    let result = verify_ed25519_consistency(&d_bytes, &wrong_x_bytes);
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
...
fn test_validate_ed25519_consistency() {
    ...
    let result = verify_ed25519_consistency(&d_bytes, x_bytes);
    assert!(result.is_ok());
}
...
fn test_validate_x25519_consistency_mismatch() {
    ...
    let result = verify_x25519_consistency(&d_bytes, public.as_bytes());
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(msg.contains("key pair inconsistency"), "got: {msg}");
}
