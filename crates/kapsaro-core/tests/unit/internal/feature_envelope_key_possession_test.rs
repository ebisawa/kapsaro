...
use crate::format::signature::{
    build_artifact_signature_input, build_key_possession_mac_message, ArtifactBodyBytes,
};
...
fn key_possession_mac_message_uses_domain_and_framed_fields() {
    ...
    let body = ArtifactBodyBytes::from_bytes(body_bytes);
    let message = build_key_possession_mac_message(&body, "XY");
    let expected = format!("{MAC_DOMAIN_KEY_POSSESSION_V1}3:abc2:XY");
...
fn key_possession_sig_input_uses_separate_domain_and_framed_fields() {
    let proof =
        KeyPossessionProof::parse("hmac-sha256:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
            .unwrap();
    let body = ArtifactBodyBytes::from_bytes(b"body");
    ...
    let expected = format!(
        "{SIG_DOMAIN_ARTIFACT_SIGNATURE_V1}{}:{header}4:body{}:{}",
        header.len(),
        proof.as_str().len(),
        proof.as_str()
    );
...
fn artifact_signature_input_binds_signature_header_values() {
    ...
    .unwrap();
    let other_alg =
        build_artifact_signature_input("future-ed25519", SIGNER_KID, &body, proof.as_str())
            .unwrap();
