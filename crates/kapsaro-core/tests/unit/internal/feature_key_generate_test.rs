...
use crate::feature::key::public_key_document::{build_public_key, PublicKeyDocumentParams};
...
fn test_derive_key_from_ssh_preserves_ssh_backend_errors() {
    ...
    let result = crate::feature::key::protection::key_derivation::derive_key_from_ssh(
...
fn test_derive_key_from_ssh_maps_non_deterministic_error() {
    ...
    let result = crate::feature::key::protection::key_derivation::derive_key_from_ssh(
        ...
        &crate::crypto::types::primitives::HkdfSalt::new([9u8; 32]),
...
fn test_generate_key_rejects_skipped_determinism() {
    ...
    let backend: Box<dyn SignatureBackend> =
        Box::new(crate::io::ssh::backend::ssh_keygen::SshKeygenBackend::new(
            Box::new(ssh_keygen),
            descriptor,
        ));
    let fingerprint =
        crate::io::ssh::protocol::fingerprint::build_sha256_fingerprint(ssh_pub_content.trim())
            .unwrap();
    ...
    let created_at = crate::support::time::format_timestamp_rfc3339(now).unwrap();
    let expires_at =
        crate::support::time::format_timestamp_rfc3339(now + time::Duration::days(365)).unwrap();

    let result = crate::feature::key::generate::generate_key(KeyGenerationOptions {
