...
use crate::feature::key::protection::key_derivation::{build_sign_message, derive_key_from_ssh};
...
use crate::model::wire::{
    algorithm, context::SSHSIG_MESSAGE_PREFIX_PRIVATE_KEY_PROTECTION_V1, format,
};
use std::cell::Cell;
...
impl SignatureBackend for FixedSignature {
    fn sign_sshsig(
        ...
        Ok(Ed25519RawSignature::new(self.0))
    }
}
...
fn build_ssh_private_key_with_plaintext_json(plaintext_json: &[u8]) -> PrivateKey {
    ...
    let enc_key = derive_key_from_ssh(
        kid,
        &encode_base64url_nopad(ikm_salt.as_bytes()),
        &hkdf_salt,
        &FixedSignature([0xAB; 64]),
        TEST_SSH_PUBKEY,
    )
    .unwrap();
...
fn test_build_sign_message_format() {
    ...
    let expected = format!(
        "{}\n{}",
        SSHSIG_MESSAGE_PREFIX_PRIVATE_KEY_PROTECTION_V1, ikm_salt
    );
...
#[test]
fn test_ssh_key_derivation_length_determinism_and_kid_binding() {
    let salt = HkdfSalt::new([1; 32]);
    let ikm_salt = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
    let derive = |kid| {
        derive_key_from_ssh(
            kid,
    ...
    let first = derive("7M2Q9D4R1H8VW6PKT3XNC5JY2F9AR8GD");
    let repeated = derive("7M2Q9D4R1H8VW6PKT3XNC5JY2F9AR8GD");
    let other = derive("7M2Q9D4R1H8VW6PKT3XNC5JY2F9AR8GE");
