...
use crate::feature::key::protection::password_encryption::{
...
fn build_password_private_key_with_plaintext_json(
    ...
    let enc_key = derive_key_from_password(password, &ikm_salt, &hkdf_salt, TEST_KID).unwrap();
...
fn test_password_encrypt_decrypt_roundtrip() {
    ...
    let decrypted = decrypt_private_key_with_password(&encrypted, &password)
...
fn test_password_encrypt_wrong_password_fails() {
    ...
    let result = decrypt_private_key_with_password(&encrypted, &wrong_password);
...
fn test_password_decrypt_sanitizes_plaintext_deserialize_error() {
    ...
    let err = decrypt_private_key_with_password(&private_key, &password)
...
fn test_password_encrypt_preserves_metadata() {
    ...
    assert_eq!(encrypted.protected.format, format::PRIVATE_KEY_V1);
}
...
fn test_password_decrypt_rejects_sshsig_key() {
    ...
    let result = decrypt_private_key_with_password(&private_key, &secret("test-password"));
