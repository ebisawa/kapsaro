...
use super::{resolve_trust_store_write, TrustStoreWrite};
...
fn test_changed_content_is_saved_whatever_the_stored_signer_is() {
    assert_eq!(
        resolve_trust_store_write(true, Some(STORED_SIGNER_KID), CURRENT_SIGNER_KID),
        TrustStoreWrite::Save
    );
    assert_eq!(
        resolve_trust_store_write(true, Some(CURRENT_SIGNER_KID), CURRENT_SIGNER_KID),
        TrustStoreWrite::Save
    );
    assert_eq!(
        resolve_trust_store_write(true, None, CURRENT_SIGNER_KID),
        TrustStoreWrite::Save
    );
}
...
fn test_unchanged_content_signed_by_another_key_is_resigned() {
    assert_eq!(
        resolve_trust_store_write(false, Some(STORED_SIGNER_KID), CURRENT_SIGNER_KID),
...
fn test_unchanged_content_signed_by_the_current_key_is_skipped() {
    assert_eq!(
        resolve_trust_store_write(false, Some(CURRENT_SIGNER_KID), CURRENT_SIGNER_KID),
        TrustStoreWrite::Skip
    );
    assert_eq!(
        resolve_trust_store_write(false, None, CURRENT_SIGNER_KID),
        TrustStoreWrite::Skip
    );
}
