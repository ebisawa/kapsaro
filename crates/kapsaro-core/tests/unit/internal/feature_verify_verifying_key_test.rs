...
//! Unit tests for feature/verify/verifying_key module
//!
//! Tests for embedded signer_pub based verifying key construction.

use crate::feature::verify::verifying_key::build_verifying_key_from_signature;
...
/// build_verifying_key_from_signature extracts key from embedded signer_pub.
#[test]
fn test_build_verifying_key_from_signature_with_signer_pub() {
    ...
    let public_key: crate::model::public_key::PublicKey = serde_json::from_str(&content).unwrap();
    ...
    let loaded = build_verifying_key_from_signature(&signature).unwrap();
...
/// build_verifying_key_from_signature succeeds even when the signer is not in
/// workspace active members because cryptographic verification is self-contained.
#[test]
fn test_build_verifying_key_from_signature_with_signer_pub_not_active_member_succeeds() {
...
/// build_verifying_key_from_signature succeeds without a workspace because
/// embedded signer_pub is the verification source.
#[test]
fn test_build_verifying_key_from_signature_with_signer_pub_no_workspace_succeeds() {
    let temp_dir = setup_test_keystore_from_fixtures(ALICE_MEMBER_HANDLE);
    let keystore_root = temp_dir.path().join("keys");
...
/// build_verifying_key_from_signature returns error when kid in signature
/// does not match kid in embedded signer_pub
#[test]
fn test_build_verifying_key_from_signature_kid_mismatch() {
    let (_temp_dir, workspace_dir) = setup_test_workspace_from_fixtures(&[ALICE_MEMBER_HANDLE]);
    ...
    let result = build_verifying_key_from_signature(&signature);
