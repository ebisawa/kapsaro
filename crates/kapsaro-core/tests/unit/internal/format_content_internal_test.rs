...
fn kv_enc_detect_rejects_json() {
    let result = KvEncContent::detect(r#"{"format":"kapsaro:format:file-enc@1"}"#.to_string());
    assert!(result.is_err());
}

#[test]
fn encrypted_content_detect_rejects_unknown() {
    let result = EncContent::detect_with_source("random text".to_string(), "encrypted content");
    assert!(result.is_err());
}
...
fn encrypted_content_detect_rejects_json_exceeding_depth_limit() {
    let result =
        EncContent::detect_with_source(deeply_nested_json(MAX_JSON_DEPTH + 1), "encrypted content");
    assert!(result.is_err());
...
fn file_enc_schema_error_includes_source_name() {
    let content = r#"{"protected":{"format":"kapsaro:format:file-enc@1"}}"#;
    let file =
        FileEncContent::new_unchecked_with_source(content.to_string(), ".kapsaro/secrets/app.json");

    let error = file.parse().unwrap_err();
    let message = error.format_user_message();

    assert!(message.contains("Invalid kapsaro document"));
    assert!(message.contains("Source: .kapsaro/secrets/app.json"));
...
fn kv_enc_schema_error_includes_source_name_and_token_context() {
    ...
    let content =
        format!(":KAPSARO_KV 1\n:HEAD {head_token}\n:WRAP {wrap_token}\n:SIG {signature_token}\n");
    let kv = KvEncContent::new_unchecked_with_source(content, ".kapsaro/secrets/default.kvenc");

    let error = kv.parse().unwrap_err();
    let message = error.format_user_message();

    assert!(message.contains("Invalid kapsaro document"));
    assert!(message.contains("Source: .kapsaro/secrets/default.kvenc (SIG token)"));
