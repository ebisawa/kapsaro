...
fn test_parse_header_v1() {
    let parsed = KvEncParser::parse_line(":KAPSARO_KV 1").unwrap();
    assert_eq!(
        parsed,
        KvEncLine::Header {
...
fn test_parse_header_invalid_version() {
    assert!(KvEncParser::parse_line(":KAPSARO_KV 999").is_err());
}

#[test]
fn test_parse_header_rejects_zero_padded_version() {
    assert!(KvEncParser::parse_line(":KAPSARO_KV 01").is_err());
}

#[test]
fn test_parse_header_rejects_signed_version() {
    assert!(KvEncParser::parse_line(":KAPSARO_KV +1").is_err());
}

#[test]
fn test_parse_header_rejects_trailing_space_after_version() {
    assert!(KvEncParser::parse_line(":KAPSARO_KV 1 ").is_err());
}
