...
/// A recipient set hash is 32 bytes of base64url, so its final character can
/// only carry the two bits the encoding leaves in use. A hash whose trailing
...
fn test_trust_store_schema_rejects_recipient_set_hash_with_non_zero_trailing_bits() {
    ...
    let error = validator.validate_trust_store(&doc).unwrap_err();
    let message = error.format_user_message();
    assert!(message.contains("recipient_set_hash"), "got: {message}");
...
fn test_trust_store_schema_rejects_unsupported_signature_algorithm() {
    ...
    let error = validator.validate_trust_store(&doc).unwrap_err();
    let message = error.format_user_message();
    assert!(message.contains("signature.alg"), "got: {message}");
}
...
fn test_trust_store_schema_rejects_signature_that_is_not_an_ed25519_signature() {
    ...
    let error = validator.validate_trust_store(&doc).unwrap_err();
    let message = error.format_user_message();
    assert!(message.contains("signature.sig"), "got: {message}");
}
...
fn test_verify_trust_store_rejects_semantically_invalid_timestamp() {
    ...
    let signer_keys = load_signer_keys(&home, &doc);
...
fn test_verify_trust_store_rejects_a_document_owned_by_another_member() {
    ...
    let signer_keys =
        load_signer_key_snapshot(&keystore, &other_owner, document_signer_kid(&doc).as_ref())
            .unwrap();
...
fn load_signer_keys(home: &tempfile::TempDir, doc: &TrustStoreDocument) -> SignerKeySnapshot {
    let keystore = KeystoreAccess::open(home.path().join("keys")).unwrap();
    let owner = MemberHandle::try_from(ALICE_MEMBER_HANDLE).unwrap();
    load_signer_key_snapshot(&keystore, &owner, document_signer_kid(doc).as_ref()).unwrap()
}
