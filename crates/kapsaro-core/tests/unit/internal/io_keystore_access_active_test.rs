...
use crate::test_utils::{
    ensure_local_state_dir, local_state_temp_dir, save_local_state_file,
    setup_test_keystore_from_fixtures, ALICE_MEMBER_HANDLE, BOB_MEMBER_HANDLE,
};
...
fn test_set_and_load_active_kid() {
    ...
    let access = KeystoreAccess::ensure(keystore_root).unwrap();
...
fn test_load_active_kid_invalid_format() {
    ...
    ensure_local_state_dir(active_path.parent().unwrap());
    save_local_state_file(&active_path, invalid_kid);
...
fn test_resolve_kid_without_an_active_marker_ranks_an_undated_key_last() {
    let temp = local_state_temp_dir();
    let access = KeystoreAccess::ensure(temp.path()).unwrap();
...
fn test_load_active_kid_requires_the_canonical_form() {
    ...
    ensure_local_state_dir(active_path.parent().unwrap());
    save_local_state_file(&active_path, "7m2q-9d4r-1h8v-w6pk-t3xn-c5jy-2f9a-r8gd\n");
...
fn test_load_active_kid_reads_a_blank_marker_as_no_active_key() {
    for content in ["", "  \n"] {
        ...
        let active_path = keystore_root.join(member.as_str()).join("active");
        ensure_local_state_dir(active_path.parent().unwrap());
        save_local_state_file(&active_path, content);
