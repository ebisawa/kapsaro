...
use crate::test_utils::{
    ensure_local_state_dir, local_state_temp_dir, save_local_state_file, BOB_MEMBER_HANDLE,
};
...
fn test_list_kids() {
    ...
    let member_path = keystore_root.join(member_handle);
    ensure_local_state_dir(&member_path.join(kid1));
    ensure_local_state_dir(&member_path.join(kid2));
...
fn test_remove_key_names_the_removed_key_when_the_namespace_turns_unsafe() {
    ...
    let error = access
        .remove_key_with_validation(&member, &kid, |_| {
            ...
            save_local_state_file(&shadowing_entry, "not a key directory");
            Ok(())
        })
        .unwrap_err();
