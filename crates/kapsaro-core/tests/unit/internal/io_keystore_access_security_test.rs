...
use crate::test_utils::{
    ensure_local_state_dir, local_state_temp_dir, setup_test_keystore_from_fixtures,
    ALICE_MEMBER_HANDLE,
};
...
#[cfg(unix)]
use crate::config::resolution::global::ensure_home;
#[cfg(unix)]
...
fn test_keystore_access_open_from_home_warns_about_group_writable_ancestor() {
    ...
    ensure_local_state_dir(&shared.join("home").join("keys"));
...
fn test_keystore_creation_warns_about_world_writable_ancestor() {
    let temp = local_state_temp_dir();
    let shared = temp.path().join("shared");
    ensure_local_state_dir(&shared);
    fs::set_permissions(&shared, fs::Permissions::from_mode(0o777)).unwrap();

    let guard = LocalStateWarningGuard::new();
    let home = ensure_home(&shared.join("home")).unwrap();
    KeystoreAccess::ensure_from_anchored_home(&home).unwrap();
...
fn test_keystore_access_open_from_home_allows_ancestor_symlink() {
    ...
    ensure_local_state_dir(&home.join("keys"));
...
fn test_keystore_access_lists_validated_member_handles() {
    ...
    let access = KeystoreAccess::ensure(&root).unwrap();
...
fn test_keystore_access_list_kids_ignores_key_pair_staging() {
    ...
    let writer = KeystoreAccess::ensure(target.path().join("keys")).unwrap();
...
fn test_concurrent_same_kid_publication_keeps_one_complete_pair() {
    ...
    KeystoreAccess::ensure(&keys_root).unwrap();
...
fn test_keystore_access_list_kids_ignores_active_marker_staging() {
    let temp = local_state_temp_dir();
    let root = temp.path().join("keys");
    let writer = KeystoreAccess::ensure(&root).unwrap();
...
fn test_keystore_access_list_kids_ignores_unrelated_hidden_directory() {
    ...
    let access = KeystoreAccess::ensure(&root).unwrap();
    let member = MemberHandle::try_from(ALICE_MEMBER_HANDLE).unwrap();
    ensure_local_state_dir(&root.join(member.as_str()).join(".tmp-stale"));
...
fn test_keystore_access_mutation_failure_survives_a_later_namespace_failure() {
    ...
    #[cfg(unix)]
    set_owner_only(&target);
    let access = KeystoreAccess::ensure(target.path().join("keys")).unwrap();
...
isolated_umask_test! {
    ...
    fn test_keystore_access_create_restricts_the_directories_it_creates() {
        ...
        with_umask(0o022, || {
            KeystoreAccess::ensure(&root).unwrap();
        });
...
fn test_keystore_creation_warns_about_insecure_home_permissions() {
    ...
    let guard = LocalStateWarningGuard::new();
    let opened = ensure_home(&home).unwrap();
    KeystoreAccess::ensure_from_anchored_home(&opened).unwrap();
...
fn test_keystore_access_load_optional_public_key_missing_member_returns_none() {
    let temp = local_state_temp_dir();
    let access = KeystoreAccess::ensure(temp.path().join("keys")).unwrap();
    let member = MemberHandle::try_from(ALICE_MEMBER_HANDLE).unwrap();
    let kid = Kid::try_from("00000000000000000000000000000000").unwrap();
...
fn test_keystore_access_ignores_leftover_staging_entry_in_member_directory() {
    ...
    let access = KeystoreAccess::ensure(&root).unwrap();
    let member = MemberHandle::try_from(ALICE_MEMBER_HANDLE).unwrap();
    ensure_local_state_dir(&root.join(member.as_str()).join(STAGING_DIR_NAME));
...
fn test_abandoned_key_staging_directory_is_removed_with_all_of_its_entries() {
    ...
    let writer = KeystoreAccess::ensure(&keystore_root).unwrap();
