...
use crate::service_test_utils::build_test_private_key_document;
...
fn test_resolve_kid_with_override() {
    ...
    let keystore_root = get_keystore_root_from_base(temp_dir.path());
...
fn test_find_member_by_kid_reports_an_unknown_kid_as_not_found() {
    let temp_dir = local_state_temp_dir();
    let access = KeystoreAccess::ensure(temp_dir.path()).unwrap();
