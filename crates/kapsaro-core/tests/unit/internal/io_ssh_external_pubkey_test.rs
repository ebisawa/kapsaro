...
const VALID_ED25519_KEY: &str = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA test@example.com";
...
/// A `.pub` file holding more than one key does not say which key the signer
/// meant, and taking the first would sign with a key the operator did not name.
#[test]
fn test_load_ssh_public_key_file_rejects_two_key_lines_error() {
    let temp_dir = TempDir::new().unwrap();
    let pub_path = temp_dir.path().join("test.pub");

    std::fs::write(
        &pub_path,
        format!("{}\n{}\n", VALID_ED25519_KEY, VALID_ED25519_KEY_2),
    )
    .unwrap();
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("SSH public key file must contain exactly one key line"));
}

/// Blank lines around the single key carry no second key, so the file still
/// names one key and is read.
#[test]
fn test_load_ssh_public_key_file_accepts_blank_lines_around_one_key() {
    let temp_dir = TempDir::new().unwrap();
    let pub_path = temp_dir.path().join("test.pub");
