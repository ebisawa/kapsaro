...
/// An agent named inside a block for one host says nothing about the hosts
/// kapsaro signs for, so nothing is resolved from it.
#[test]
fn test_parse_identity_agent_ignores_a_specific_host_block() {
    let config = r#"
Host work-server
    IdentityAgent /work/sock
"#;
    let result = parse(config).unwrap();
    assert!(result.is_none());
}

/// `HostName` opens no block. Reading it as one would end the `Host *` block it
/// stands in and send that block's agent to the global scope.
#[test]
fn test_parse_identity_agent_reads_host_star_past_a_hostname_line() {
    let config = r#"
IdentityAgent /global/sock
Host github.com
    IdentityAgent /other/sock
...
"#;
    let result = parse(config).unwrap();
    let path = result.expect("the Host * block names an agent");
    assert!(path.to_string_lossy().contains("host_star"));
}

/// Keywords are matched without regard to case, so a lower-case block is a
/// block and the agent inside it wins over the one at global scope.
#[test]
fn test_parse_identity_agent_reads_a_lowercase_host_star_block() {
    let config = r#"
IdentityAgent /global/sock
host *
    IdentityAgent "~/host_star/sock"
