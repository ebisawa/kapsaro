...
/// Every blob reaching this type comes from an SSHSIG `signature` field, which
/// carries its algorithm identifier, so 64 bare bytes name no algorithm and are
/// read as wire framing that does not decode.
#[test]
fn test_ssh_signature_blob_rejects_unframed_64_bytes_error() {
    let mut raw = [0u8; 64];
    ...
    assert!(blob.extract_ed25519_raw().is_err());
}
