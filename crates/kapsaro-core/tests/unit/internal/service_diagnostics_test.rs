...
use crate::test_utils::{
    ensure_local_state_dir, local_state_temp_dir, member_handle, permission_denial_can_be_staged,
    save_local_state_file, setup_member_key_context, setup_test_workspace_from_fixtures, EnvGuard,
...
fn ensure_workspace_dirs(workspace: &Path) {
...
fn test_doctor_ci_invalid_env_key_reports_fail_and_strict_warning() {
    let checks = check_ci_readiness(DoctorCiReadiness::active(
        false,
        Some("invalid environment key".to_string()),
    ));
...
#[test]
fn test_doctor_ci_unreadable_strict_key_checking_reports_fail() {
    let checks = check_ci_readiness(DoctorCiReadiness::active_with_invalid_strict_key_checking(
        "Invalid KAPSARO_STRICT_KEY_CHECKING value 'maybe'. Expected 'yes' or 'no'.".to_string(),
        None,
    ));

    let check = find_check(&checks, "ci.strict_key_checking", DoctorStatus::Fail);

    assert!(check
        .reason_line()
        .is_some_and(|reason| reason.contains("Expected 'yes' or 'no'")));
}
...
/// The caller names the owner, so a handle it resolved is the one every later
/// check is bound to, without the diagnosis reading the configuration itself.
#[test]
fn test_doctor_uses_the_caller_resolved_owner_for_the_keystore_checks() {
    ...
    let owner_check = find_check(report.checks(), "keystore.member", DoctorStatus::Ok);
...
fn test_doctor_reports_invalid_trust_store_signature_as_failure() {
    ...
    ensure_local_state_dir(trust_path.parent().unwrap());
    save_local_state_file(&trust_path, "{invalid-json");
...
/// Two active member documents sharing one kid leave it undecided which member
/// a signer or recipient with that kid is. The member set is reported as
/// unusable rather than resolved to whichever document was read last, so the
/// signer and recipient judgments are left out the same way an unreadable set
/// leaves them out.
#[test]
fn test_doctor_reports_duplicate_active_kid_instead_of_judging_against_one_of_them() {
    let (home, workspace) = setup_test_workspace_from_fixtures(&[ALICE_MEMBER_HANDLE]);
    fs::write(
    ...
    fs::copy(active_file, workspace.join("members/active/duplicate.json")).unwrap();
