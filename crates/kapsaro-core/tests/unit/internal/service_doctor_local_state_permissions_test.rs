...
use crate::test_utils::{
    ensure_local_state_dir, local_state_temp_dir, permission_denial_can_be_staged,
    save_local_state_file, with_temp_cwd,
};
...
fn test_local_state_permissions_report_an_owner_only_tree_as_ok() {
    ...
    save_local_state_file(&home.path().join("config.toml"), "member_handle = \"a\"\n");
...
fn test_local_state_permissions_report_one_check_per_reachable_entry() {
    ...
    ensure_local_state_dir(&keys);
    let document = keys.join("private.json");
    save_local_state_file(&document, "{}");
...
fn test_local_state_ancestor_owner_skips_an_ancestry_it_cannot_walk() {
    ...
    ensure_local_state_dir(&blocked);
