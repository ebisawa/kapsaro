...
//! Unit tests for service/errors.rs helpers.

use crate::feature::kv::error::build_key_not_found_error;
use crate::service::errors::{build_kv_key_not_found_error, serialize_to_json_value};
use crate::{Error, ErrorKind};
use serde::{Serialize, Serializer};
use std::path::Path;

#[test]
fn test_build_kv_key_not_found_error_names_the_file_holding_the_missing_key() {
    let err = build_key_not_found_error("foo");
...
#[test]
fn test_build_kv_key_not_found_error_passthrough_for_an_unmarked_look_alike_message() {
    let err = Error::build_invalid_operation_error("Key 'foo' not found");
    let wrapped = build_kv_key_not_found_error(err, Path::new("/tmp/x.kvenc"), "foo");
    assert_eq!(wrapped.kind(), ErrorKind::InvalidOperation);
    assert_eq!(wrapped.format_user_message(), "Key 'foo' not found");
}
