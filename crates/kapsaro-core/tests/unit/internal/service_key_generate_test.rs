...
use super::{
    ...
    publish_generated_key, KeyGenerationHome, LocalKeyGenerationOptions,
};
...
fn test_ensure_kid_not_in_keystore_passes_when_absent() {
    ...
    let access = KeystoreAccess::ensure(dir.path()).unwrap();
...
fn test_key_generation_home_creates_the_keystore_through_an_explicit_home_symlink() {
    ...
    ensure_local_state_dir(&outside);
...
fn test_a_key_lands_in_the_home_fixed_at_the_start_after_the_path_is_repointed() {
    ...
    let home = temp.path().join("home");
    ensure_local_state_dir(&started_in);
    ensure_local_state_dir(&repointed_to);
