...
fn test_workspace_read_session_keeps_the_matching_local_state_capability_after_repointing() {
    ...
    crate::test_utils::ensure_local_state_dir(&replacement);
...
#[test]
fn test_output_recipient_preflight_requires_review_for_other_member_key() {
    let (home, workspace) =
        setup_test_workspace_from_fixtures(&[ALICE_MEMBER_HANDLE, BOB_MEMBER_HANDLE]);
    ...
    let decision = evaluator
        .preflight_output_recipient_keys(&recipients, &self_trust)
        .expect("preflight output recipients");

    let TrustDecision::ReviewRequired(requests) = decision else {
        panic!("an unknown other-member recipient key must require review");
    };
...
#[test]
fn test_preflight_read_keeps_unresolved_recipient_kids_when_review_is_skipped() {
    let (home, workspace) =
        setup_test_workspace_from_fixtures(&[ALICE_MEMBER_HANDLE, BOB_MEMBER_HANDLE]);
    let (artifact, _signer_ctx) = load_file_artifact(
        &home,
