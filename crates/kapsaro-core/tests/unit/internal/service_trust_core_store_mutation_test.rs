...
use crate::api::key::{KeyContext, KeyContextOptions, LocalKeyStore, MemberHandle};
use crate::api::ssh::{SshRawSignature, SshSignatureBackend};
use crate::api::trust::{ApprovalConflictHandling, TrustApproval};
...
use crate::test_utils::{ensure_local_state_dir, member_handle, setup_member_key_context};
use crate::ErrorKind;
use serde_json::Value;
use tempfile::TempDir;

use crate::service_test_utils::{rotate_active_key, save_trust_store_signed_by_active_key};
...
fn build_trust_store(home_path: &Path, owner_handle: &str) -> crate::api::trust::LocalTrustStore {
    crate::api::trust::LocalTrustStore::open(home_path, member(owner_handle))
...
fn seed_trust_store(trust_store: &crate::api::trust::LocalTrustStore, key_ctx: &KeyContext) {
    trust_store
...
fn read_stored_trust_store(trust_store: &crate::api::trust::LocalTrustStore) -> Value {
...
fn apply_approvals_resigns_for_a_rotated_key_without_touching_updated_at() {
    ...
    save_trust_store_signed_by_active_key(&temp, ALICE, STORED_AT, Vec::new(), Vec::new());
    let trust_store = build_trust_store(temp.path(), ALICE);
