...
use crate::service_test_utils::{
    build_known_key, build_recipient_set, build_test_command_options,
    build_test_trust_command_session, load_test_trust_store, rotate_active_key,
...
fn save_signed_trust_store_with_recipient_sets(
    ...
    save_trust_store_signed_by_active_key(
        ...
        STORED_AT,
        vec![
            build_known_key(KID_OLD, "bob@example.com", Some("2026-01-01T00:00:00Z")),
            build_known_key(
                KID_FRACTIONAL,
                "charlie@example.com",
                Some("2026-01-01T00:00:00.1Z"),
            ),
            build_known_key(KID_NEW, "dave@example.com", Some("2026-06-01T00:00:00Z")),
        ],
        recipient_sets,
    );
}
...
fn test_purge_candidate_lists_require_an_existing_trust_store() {
    for create_trust_directory in [false, true] {
        ...
        if create_trust_directory {
            crate::test_utils::ensure_local_state_dir(&home.path().join("trust"));
        }
        let session = build_test_trust_command_session(&home, ALICE_MEMBER_HANDLE);
