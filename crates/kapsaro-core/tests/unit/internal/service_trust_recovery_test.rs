...
use crate::test_utils::{
    ensure_local_state_dir, member_handle, save_local_state_file, setup_member_key_context_at,
};
...
fn test_list_command_reset_uses_resolved_owner_and_trust_directory() {
    ...
    ensure_local_state_dir(alice_path.parent().unwrap());
    save_local_state_file(&alice_path, "invalid-alice");
    save_local_state_file(&bob_path, "invalid-bob");
    let command =
        resolve_trust_list_command(home.path(), member_handle("alice@example.com")).unwrap();
    let token = observe_trust_store_recovery_from_list_command(&command);
    ...
    ensure_local_state_dir(&replacement_trust);
    let replacement_alice = replacement_trust.join("alice@example.com.json");
    save_local_state_file(&replacement_alice, "replacement-alice");
...
fn test_list_command_reset_preserves_target_changed_after_plan() {
    ...
    save_local_state_file(&trust_path, "invalid");
    let command =
        resolve_trust_list_command(home.path(), member_handle("alice@example.com")).unwrap();
    let token = observe_trust_store_recovery_from_list_command(&command);
    let error = list_known_keys_command(&command).unwrap_err();
    let plan =
        build_trust_store_reset_plan_from_list_command(&command, token, error, true).unwrap();
    save_local_state_file(&trust_path, "replacement");
...
fn test_execution_reset_deletes_only_original_home_target_after_path_swap() {
    ...
    ensure_local_state_dir(original_target.parent().unwrap());
    ensure_local_state_dir(replacement_target.parent().unwrap());
    save_local_state_file(&original_target, "invalid-original");
    save_local_state_file(&replacement_target, "replacement-bytes");
    let execution = build_execution(&home, "alice@example.com");
    let session = build_session(&home, "alice@example.com");
    let token = observe_trust_store_recovery_from_session(&execution);
    ...
    ensure_local_state_dir(&opened_home.join("trust"));
    let replaced_original_target =
        get_trust_store_file_path(&opened_home, &member_handle("alice@example.com"));
    save_local_state_file(&replaced_original_target, "replacement-directory-bytes");
    let plan = build_trust_store_reset_plan_from_session(&execution, token, error, true).unwrap();
    execute_trust_store_reset(&plan).unwrap();
...
#[test]
fn test_evaluate_trust_store_reset_ignores_a_coded_operation_error() {
    let error = crate::Error::build_local_state_path_unsafe_error("unsafe local state".to_string());

    assert!(evaluate_trust_store_reset(&error).is_none());
}
