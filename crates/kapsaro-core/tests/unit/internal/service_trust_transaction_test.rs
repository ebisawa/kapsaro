...
use super::{
    commit_trust_store_mutation, evaluate_locked_failure, stale_attempt_leaves_room_to_reobserve,
    ObservedTrustStore, TrustStoreCommitGate, TrustStoreContentSnapshot, MERGED_OBSERVATION_LIMIT,
};
...
use crate::service_test_utils::save_trust_store_signed_by_active_key;
...
fn test_unparsable_content_under_the_lock_comes_back_for_the_gate() {
    let classified = evaluate_locked_failure(Error::build_parse_error(
...
fn test_unverifiable_content_under_the_lock_comes_back_for_the_gate() {
    let classified = evaluate_locked_failure(Error::build_verification_error(
...
fn test_io_failure_under_the_lock_is_reported_as_itself() {
    match evaluate_locked_failure(Error::build_io_error("Disk read failed")) {
        Err(error) => {
...
fn test_unsafe_local_state_path_under_the_lock_is_reported_as_itself() {
    match evaluate_locked_failure(Error::build_local_state_path_unsafe_error(
