...
fn test_load_ssh_public_key_from_keygen_uses_sanitized_env_with_optional_socket() {
    ...
    assert!(!output.contains("KAPSARO_PRIVATE_KEY=sensitive"));
}

#[test]
fn test_default_ssh_add_sets_resolved_socket_without_inheriting_kapsaro() {
    let _guard = EnvGuard::new(&[
        "HOME",
        "PATH",
        "SSH_AUTH_SOCK",
...
fn test_default_ssh_keygen_sign_with_public_key_uses_agent_stdin_stdout() {
    let _guard = EnvGuard::new(&["HOME", "PATH", "SSH_AUTH_SOCK", "KAPSARO_PRIVATE_KEY"]);
    let fake_home = TempDir::new().unwrap();
    std::env::set_var("HOME", fake_home.path());
    std::env::set_var("PATH", "/usr/bin");
    std::env::set_var("SSH_AUTH_SOCK", "/tmp/agent.sock");
...
if [ \"${{KAPSARO_PRIVATE_KEY:-}}\" = \"sensitive\" ]; then\n\
...
if [ \"$6\" != \"kapsaro-key-protection\" ] || [ \"$8\" != \"kapsaro-key-protection\" ]; then\n\
