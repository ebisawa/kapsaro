...
fn derive_enc_key(raw_sig: &[u8], salt: &HkdfSalt, kid: &str) -> kapsaro_core::Result<XChaChaKey> {
    let ikm = Ikm::from(raw_sig);
    let info = Info::from_string(&format!("{}:{}", HKDF_INFO_PRIVATE_KEY_SSHSIG_V1, kid));
...
fn test_derive_enc_key_info_format() {
    ...
    // The info should be "kapsaro:sshsig-private-key-enc@1:{kid}"
