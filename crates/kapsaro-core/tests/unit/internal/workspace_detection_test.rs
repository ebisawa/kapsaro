...
fn build_workspace(root: &TempDir) -> (PathBuf, PathBuf) {
    ...
    let workspace_root = repo_root.join(".kapsaro");
...
#[test]
fn test_detect_workspace_without_git_uses_current_dot_kapsaro() {
    let temp = TempDir::new().unwrap();
...
fn test_detect_workspace_in_git_worktree() {
    ...
    // Main repository with .git directory and .kapsaro workspace
    fs::create_dir_all(main_repo.join(".git/worktrees/my-worktree")).unwrap();
    fs::create_dir_all(main_repo.join(".kapsaro/members/active")).unwrap();
    fs::create_dir_all(main_repo.join(".kapsaro/secrets")).unwrap();
    ...
    let workspace = result.unwrap();
    let expected = main_repo.join(".kapsaro").canonicalize().unwrap();
...
#[test]
fn test_check_workspace_kapsaro_subdir_requires_active() {
    let tmp = TempDir::new().unwrap();
    fs::create_dir_all(tmp.path().join(".git")).unwrap();

    // .kapsaro/ に members/ と secrets/ だけ → workspace と認識されない
    fs::create_dir_all(tmp.path().join(".kapsaro/members")).unwrap();
    fs::create_dir_all(tmp.path().join(".kapsaro/secrets")).unwrap();
    let result = detect_workspace_root(tmp.path());
    assert!(result.is_err());

    // .kapsaro/members/active/ も追加 → workspace と認識される
    fs::create_dir_all(tmp.path().join(".kapsaro/members/active")).unwrap();
    let result = detect_workspace_root(tmp.path());
    assert!(result.is_ok());
    let expected = tmp.path().join(".kapsaro").canonicalize().unwrap();
...
fn test_check_workspace_rejects_symlinked_members_active() {
    ...
    let kapsaro_dir = tmp.path().join(".kapsaro");
    fs::create_dir_all(kapsaro_dir.join("members")).unwrap();
    fs::create_dir_all(kapsaro_dir.join("secrets")).unwrap();

    let outside = tmp.path().join("external_active");
    fs::create_dir(&outside).unwrap();
    symlink(&outside, kapsaro_dir.join("members").join("active")).unwrap();
...
fn test_check_workspace_rejects_symlinked_secrets() {
    ...
    let kapsaro_dir = tmp.path().join(".kapsaro");
    fs::create_dir_all(kapsaro_dir.join("members/active")).unwrap();

    let outside = tmp.path().join("external_secrets");
    fs::create_dir(&outside).unwrap();
    symlink(&outside, kapsaro_dir.join("secrets")).unwrap();
