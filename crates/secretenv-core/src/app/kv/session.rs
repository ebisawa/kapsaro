...
impl KvFileTarget {
    pub fn resolve(options: &CommonCommandOptions, file_name: Option<&str>) -> Result<Self> {
...
pub struct KvFileSession {
    pub target: KvFileTarget,
    content: String,
}

impl KvFileSession {
    pub fn load(options: &CommonCommandOptions, file_name: Option<&str>) -> Result<Self> {
        let target = KvFileTarget::resolve(options, file_name)?;
        Self::load_target(target)
    }

    pub fn load_target(target: KvFileTarget) -> Result<Self> {
    ...
    pub fn kv_content(&self) -> KvEncContent {
...
impl KvCommandSession {
    ...
    pub fn load_required_file(&self) -> Result<KvFileSession> {
        KvFileSession::load_target(self.target.clone())
    }
}

pub fn load_existing_content(
    target: &KvFileTarget,
    allow_missing: bool,
