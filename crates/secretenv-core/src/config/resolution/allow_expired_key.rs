...
const ENV_VAR: &str = "SECRETENV_ALLOW_EXPIRED_KEY";
const CONFIG_KEY: &str = "allow_expired_key";
...
pub fn resolve_allow_expired_key(cli_value: Option<bool>, base_dir: Option<&Path>) -> Result<bool> {
    ...
    let resolved =
        resolve_string_with_source(None, Some(ENV_VAR), CONFIG_KEY, base_dir, Some("no".into()))?;
    let value = resolved
        .map(|(value, _)| value)
