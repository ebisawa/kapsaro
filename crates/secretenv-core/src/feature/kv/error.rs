...
pub(crate) fn map_key_not_found_error(error: Error, key: &str) -> Error {
...
#[cfg(test)]
mod tests {
    use super::{is_key_not_found_error, map_key_not_found_error};
    use crate::{Error, ErrorKind};

    #[test]
    fn test_map_key_not_found_error_preserves_matching_error_shape() {
        let error = Error::build_invalid_operation_error("Key 'DATABASE_URL' not found");
        let mapped = map_key_not_found_error(error, "DATABASE_URL");
    ...
    #[test]
    fn test_map_key_not_found_error_passes_unrelated_invalid_operation_through() {
