...
fn missing_sig_error() -> Error {
    Error::build_crypto_error("kv-enc v9 has no SIG line (v9 requires signatures)".to_string())
}
...
fn validate_unique_line(
    ...
    if count == 0 {
        return Err(Error::build_verification_error(
            ...
            format!("kv-enc v9: missing {} line", label),
        ));
    }
    if count > 1 {
        return Err(Error::build_verification_error(
            ...
            format!(
                "kv-enc v9: {} line appears {} times (must be exactly once)",
                label, count
            ),
        ));
    }
        ...
        match line {
            ...
            | KvEncLine::Header { .. } => {
                if found_sig {
                    return Err(Error::build_verification_error(
                        "E_SCHEMA_INVALID".to_string(),
                        "kv-enc v9: data lines (HEAD/WRAP/KV) must not appear after :SIG line"
                            .to_string(),
                    ));
                }
            }
        ...
        if let KvEncLine::KV { key, .. } = line {
            if !is_valid_key_name(key) {
                return Err(Error::build_verification_error(
                    "E_SCHEMA_INVALID".to_string(),
                    format!(
                        "kv-enc v9: invalid KEY format '{}' (must match ^[A-Za-z_][A-Za-z0-9_]*$)",
                        key
                    ),
                ));
            }
            if !seen_keys.insert(key.clone()) {
                return Err(Error::build_verification_error(
                    ...
                    format!(
                        "kv-enc v9: duplicate KEY '{}' (each KEY must appear only once)",
                        key
                    ),
                ));
            }
...
fn validate_kv_header_lines(logical_lines: &[(usize, &KvEncLine)]) -> Result<()> {
    validate_unique_line(
        ...
        Some(0),
        "E_SCHEMA_INVALID",
        "kv-enc v9: :SECRETENV_KV 9 must be the first line",
    )?;
    validate_unique_line(
        logical_lines,
        ...
        Some(1),
        "E_SCHEMA_INVALID",
        "kv-enc v9: :HEAD must be the second line (after :SECRETENV_KV 9)",
    )?;
    validate_unique_line(
        logical_lines,
        ...
        "kv-enc v9: :WRAP must be the third line (after :HEAD)",
    )?;
    validate_unique_line(
        logical_lines,
        ...
        "E_SCHEMA_INVALID",
        "kv-enc v9: :SIG must be the last logical line (after all KV entries)",
    )?;
    Ok(())
}
