...
//! Artifact signature input byte builders.
//!
//! Assembles domain-separated, length-framed byte strings for MAC and signature inputs.

use serde::Serialize;

use crate::format::jcs;
use crate::model::wire::context::{MAC_DOMAIN_KEY_POSSESSION_V2, SIG_DOMAIN_ARTIFACT_SIGNATURE_V2};
use crate::Result;

pub(crate) fn build_key_possession_mac_message(body_bytes: &[u8], signer_kid: &str) -> Vec<u8> {
    build_domain_framed_bytes(
        MAC_DOMAIN_KEY_POSSESSION_V2.as_bytes(),
        &[body_bytes, signer_kid.as_bytes()],
    )
}
...
pub(crate) fn build_artifact_signature_input(
    ...
    Ok(build_domain_framed_bytes(
        SIG_DOMAIN_ARTIFACT_SIGNATURE_V2.as_bytes(),
        &[header.as_slice(), body_bytes, mac_string.as_bytes()],
    ))
}
