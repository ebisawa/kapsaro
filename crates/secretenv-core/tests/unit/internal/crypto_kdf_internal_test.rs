...
fn hkdf_sha256_prk_expansion_matches_one_shot_derivation() {
    ...
    let info = Info::from_string("secretenv:test:hkdf:info");
...
fn hkdf_sha256_prk_expansion_is_bound_to_info() {
    ...
    let prk = derive_hkdf_sha256_prk(&ikm, salt.as_bytes());
    let info_a = Info::from_string("secretenv:test:hkdf:info:a");
    let info_b = Info::from_string("secretenv:test:hkdf:info:b");
