...
fn test_crypto_context_load_fails_when_public_key_mismatches_private_key() {
    ...
    let public2: PublicKey = load_public_key(&keystore_root, ALICE_MEMBER_HANDLE, &kid2).unwrap();
    ...
    let ssh_pub_for_kdf = fs::read_to_string(temp_dir.path().join(".ssh").join("test_ed25519.pub"))
        ...
        .to_string();
    let backend: Box<
        dyn secretenv_core::cli_api::test_support::storage::ssh::backend::SignatureBackend,
