...
**SecretEnv が保証しないもの:** 復号後の内部者悪用の防止、過去に開示された秘密の回収、強い前方秘匿性、TOFU（Trust On First Use）を超える本人性保証。これらは見落としではなく、明示的な非目標である。とくに、PrivateKey 保護に使う SSH 秘密鍵が漏洩した場合、その SSH 鍵で保護されているすべての SecretEnv 鍵世代も漏洩と同等に扱う必要があり、それらの鍵世代に向けた過去暗号文は復号可能になる。
...
| **利用者 / 運用責任者 / 導入判断者** | エグゼクティブサマリー、§1（概要）、§2.1〜§2.4（脅威モデル要約）、§7.8〜§9（運用前提と信頼境界）、§13（制約事項）、付録 B（運用チェック） | 自組織で安全に運用できる条件と、受け入れるべき制約を判断する |
...
したがって、SSH 秘密鍵は認証資産であるだけでなく、SecretEnv PrivateKey 復元の実効的な trust root でもある。この鍵、またはそれで署名できる agent が侵害されると、その SSH 鍵で保護されたすべての SecretEnv 鍵世代も侵害されたものとして扱う必要があり、それらの鍵世代に向けた過去暗号文は復号可能になる。
...
| **運用前提に依存するもの** | 鍵の本人性判断、`members/active` への変更レビュー、TOFU 承認、ローカル信頼ストア領域の保護、SSH 秘密鍵 / agent ソケットの保護、CI 上の安全な実行条件 |
...
SSH ベースの PrivateKey 保護は、既存 SSH 鍵の署名能力を復号能力の源泉として利用する。流れは次のとおりである。

| 段階 | 入力 | 出力 | セキュリティ上の意味 |
| --- | --- | --- | --- |
| SSHSIG 署名 | `ikm_salt` を含む署名メッセージ、namespace `secretenv-key-protection` | Ed25519 raw signature bytes | SSH 鍵を使える主体だけが次段階へ進める |
| HKDF-SHA256 | raw signature bytes、`hkdf_salt`、`kid` を含む info | `enc_key` | 鍵世代ごとに domain separation された対称鍵を導出する |
...
署名メッセージは、バージョン付き識別子 `secretenv:key-protection-ikm@5` と `ikm_salt` から構成される。`kid` は SSH 署名のメッセージ本体ではなく、後段の HKDF info に含めて domain separation に使う。

### 7.4 SSHSIG signed_data

SSH 署名は OpenSSH `PROTOCOL.sshsig` 形式に従い、namespace は `secretenv-key-protection`、hash algorithm は `sha256` を用いる。attestation 用 namespace (`secretenv-attestation`) とは明確に分離される。

### 7.5 暗号化鍵の導出

`enc_key` は `HKDF-SHA256(ikm = raw signature bytes, salt = hkdf_salt, info = "secretenv:sshsig-private-key-enc@5:{kid}")` により導出する。これは保存済みの固定鍵ではなく、暗号化時と復号時の双方で同じ SSH 署名能力から再導出される。

### 7.5.1 IKM として使う署名値の機密性
...
理由は、署名値を IKM に使う以上、非決定論的署名では暗号化時と復号時で異なる `enc_key` が導出され、復号不能になるためである。
...
AAD は `jcs(protected)` である。したがって `format`、`member_id`、`kid`、アルゴリズム識別子、fingerprint、各 salt、タイムスタンプ、有効期限を含む復号条件全体が改ざん検知の対象になる。

### 7.7.1 復号フローの読み方

ローカルキーストア保護の要点は次の 3 点である。

1. ローカルキーストアから読む場合は、対応する `public.json` も検証して `member_id` / `kid` の整合性を確認する。
2. `ikm_salt` と SSH 署名能力から IKM を再構成し、`hkdf_salt` と `kid` を含む info で `enc_key` を導出する。
3. `jcs(protected)` を AAD として復号し、ヘッダ改ざんを検知する。

このため、SSH 鍵はローカルキーストア保護の認証手段であるだけでなく、事実上の復号能力の源泉でもある。

### 7.8 信頼仮定

PrivateKey 保護は SSH 署名から IKM を導出する設計のため、`secretenv-key-protection` namespace で SSH 署名を実行できる主体は、PrivateKey の暗号化鍵を導出・復号できる。この同値性は設計上意図されたものであり、信頼境界上の重要事項である。
...
agent forwarding が有効な環境では、リモートホスト上のプロセスがローカル `ssh-agent` に署名要求を送信できるため、結果として PrivateKey 復号能力も委譲される。したがって、SecretEnv を扱う環境では agent forwarding を無効にすることが重要である。

SecretEnv は既存の SSH 認証基盤を鍵保護の trust anchor として活用する代わりに、SSH 鍵保護レベルが SecretEnv 保護レベルの上限になるというトレードオフを受け入れている。
...
この方式では、Password と `ikm_salt` から Argon2id で 32-byte IKM を導出し、その IKM と `hkdf_salt` から HKDF-SHA256 により暗号化鍵を導出する。HKDF info は `secretenv:password-private-key-enc@5:{kid}` とし、SSH ベースの経路と明確に domain separation する。
...
環境変数はプロセスメモリや CI 実行環境の可視性に依存するため、パスワード保護は主として exported blob 単体漏洩時の追加防御である。

一方、`SECRETENV_PRIVATE_KEY` と `SECRETENV_KEY_PASSWORD` を同じ secret backend に保存する構成では、backend 侵害に対する独立防御にはほぼならない。別 trust domain に分けられる場合のみ、防御層としての価値が相対的に高くなる。

#### 7.9.5 環境変数経由の鍵ロードと trusted CI 境界

環境変数経由の鍵ロードは、CI などの read-path 中心の実行文脈に限る。鍵ロード時は exported PrivateKey 自体の妥当性のみを確認し、workspace の `members/active/` を self PublicKey の探索元として使ってはならない。

この方式を許容するのは、次の条件を満たす trusted CI context に限られる。

- workflow / job 定義が maintainer 管理下にあり、攻撃者制御の PR から変更・起動できない
- checkout 対象が protected branch、protected tag、post-merge ref などの trusted ref である
- runner が trusted であり、untrusted workload と共有されない

要するに、この方式は attacker-controlled な CI 文脈では使用してはならない。
...
- PrivateKey 保護に使う SSH 秘密鍵が漏洩した場合、その SSH 鍵で保護されているすべての SecretEnv 秘密鍵世代は実質的に漏洩したものとして扱う必要があり、それらの鍵世代に向けた過去暗号文はすべて復号可能になる
...
- SSH 鍵、`ssh-agent`、または trusted CI context を自組織で適切に保護できるか
