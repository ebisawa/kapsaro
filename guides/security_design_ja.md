...
§1.2 で示した4軸から導かれる個別のセキュリティ主張を、本書で扱う形で次の表に整理する。各行は Executive Summary に挙げた主張をより詳細に展開したものであり、具体的な設計上の裏付け、前提条件、残余リスクは §3 以降で論じる。本表は §3 以降への索引として参照されたい。


| セキュリティ主張         | 主な仕組み                                       | 成立前提                              | 残余リスク                                | 詳論 |
| ---------------- | ------------------------------------------- | --------------------------------- | ------------------------------------ | ---- |
| **機密性**          | HPKE wrap + XChaCha20-Poly1305              | recipient の秘密鍵が漏洩していない            | 正当 recipient による持ち出しは防げない            | §3, §7, §8 |
| **改ざん検知**        | Ed25519 署名                                  | 署名検証が必ず実行される                      | 署名者自身が悪意を持つ場合は防げない                   | §5 |
| **署名成果物の自己完結検証** | 署名者公開鍵の埋め込み + 公開鍵文書の検証             | すべての署名成果物に署名者公開鍵が埋め込まれている | 現在のメンバーであるかどうかは別途信頼ポリシーに依存する         | §5 |
| **鍵一貫性**         | 公開鍵文書の自己署名                              | 元の秘密鍵が漏洩していない                     | 新規の悪意ある鍵作成は防げない                      | §5.5 |
| **現在有効な信頼状態の判定** | 現メンバー一覧 + 承認済み鍵キャッシュ             | リポジトリ運用統制と利用者承認が適切に機能する           | 初期受け入れ時の TOFU、リポジトリ侵害、誤承認に弱い         | §10 |
| **鍵の本人性補強**      | SSH attestation + 手動承認 + オンライン検証            | 手動承認が適切に行われる                      | 初回接触時 MITM 攻撃、GitHub / SSH 信頼基盤侵害に弱い | §2.5, §5.6, §5.7 |
| **文脈束縛**         | 成果物を文脈（ファイル・鍵世代・エントリ・プロトコル）に束縛 | 実装が仕様どおりの束縛を維持する                  | 束縛を削る実装変更で弱くなる                       | §6, §11 |
| **可搬な秘密鍵利用**     | password export または SSH ベース保護               | 信頼できる CI 実行文脈でのみ使う                | 同一 secret backend への保存は独立防御にならない     | §9 |


### 1.4 実装で死守する不変条件

§1.3 のセキュリティ主張は、実装が次の不変条件を維持して初めて成立する。各項目は、その不変条件が破られた場合にどの主張が崩れるかを示す。

- **署名検証前に復号しない**。順序を逆転させると、攻撃者が制御する暗号文をそのまま復号処理に流すことになり、改ざん検知の意味が失われる。
- **文脈束縛を実装変更で削らない**。AAD・info・signed_data から束縛要素を取り除くと、暗号学的検証は通っても文脈間の流用を排除できなくなる。
- **署名者公開鍵が埋め込まれていない成果物は安全側で拒否する**。代替の鍵探索経路を許すと、自己完結検証の前提が崩れる。
- **`SECRETENV_STRICT_KEY_CHECKING=no` の適用範囲を読み取り経路に限定する**。書き込み経路でこの効果を許すと、認可基準と承認キャッシュの役割が混ざる。
...
| 用語            | 本書での意味                                                               |
| ------------- | -------------------------------------------------------------------- |
| **埋め込み署名者公開鍵** | 各署名成果物の中に同梱される署名検証用の公開鍵文書（実装上は `signature.signer_pub`、§5 参照）。これが署名検証鍵の唯一の出所であり、外部の鍵サーバや別ファイルからの代替探索は行わない |
...
| **承認済み鍵キャッシュ** | 利用者が過去に確認した鍵を再確認なしで扱うためのローカルキャッシュ（実装上は `known_keys`、§10.4 参照） |
| **非メンバー例外受理** | 現メンバー一覧に存在しない署名者の成果物を、対話的 / 一回限り / 成果物単位で例外受理する仕組み |
| **本人性補強**     | 鍵がどの人物・アカウントに紐付くかの判断材料を増やす運用層 |
| **文脈束縛**      | 各暗号成果物を、それが属するファイル・鍵世代・エントリ・プロトコルといった文脈に暗号学的に結びつけ、コンポーネントの入れ替えや流用が成立しないようにする仕組み（個別の識別子と束縛マトリクスは §6 参照） |
| **開示履歴**      | メンバー除外後に外部システム側でのローテーションが必要になり得る秘密値を可視化するための履歴（実装上は `disclosed`、§8.7 参照） |
...
**リポジトリレベルのロールバック** Git を配布媒体として使う以上、リポジトリの正規利用者が過去のコミットから古い暗号ファイルを取得できること自体は避けられない。したがって、書き込み権限を持つ攻撃者または内部者が、過去に正当だった暗号ファイルを現在の HEAD に戻す変更をコミットするリポジトリレベルのロールバックは、SecretEnv の文脈束縛だけでは検知対象にならない。文脈束縛が保証するのは暗号ファイルの整合性と文脈整合であり、Git 履歴に対する最新性や単調な進行ではないためである。

この種のリスクは、SecretEnv の暗号設計だけでは検知・防止できない。SecretEnv は、保護ブランチ、必須レビュー、変更管理、デプロイ前確認などのリポジトリ運用により、過去版の暗号ファイルが現在の HEAD に昇格されないことを前提とする。
...
ローカル信頼ストア（`secretenv.trust.local@3`）の `signature` は、`alg` / `kid` / `sig` の表現を共有するが、**埋め込み `signer_pub` は持たない**。検証方法の例外は §5.4 末尾および §10.4 で扱う。
...
recipients（wrap 配列の recipient_handle 一覧）は payload AAD に**含めない**。
...
- 受信者の member_handle は認可上重要だが、HPKE wrap の暗号学的束縛は `kid` を基準に行う。

### 7.5 payload 暗号化

- payload ヘッダには `format = secretenv.file.payload@4`、外側と同じ `sid`、AEAD 識別子 `xchacha20-poly1305` を含める。
...
- entry AAD には、dotenv KEY を表す `k`、ファイル識別子 `sid`、プロトコル識別子 `p = secretenv:kv:payload@4` を含める。
...
- **認可判定**: current workspace の `members/active` を見て、その `(member_handle, kid)` が現在のメンバー集合に含まれるかを判定する
...
1. `(signer_pub.protected.subject_handle, signer_pub.protected.kid)` が current workspace の `members/active` に存在する
2. `signer_pub.protected.kid` がローカル信頼ストアの `known_keys` に存在する、または当該実行で利用者が手動承認する

補足:

- 実行主体自身の署名秘密鍵、またはローカルキーストア上で同一 `member_handle` に属する既存 self 鍵と一致する `signer_pub` は、1 と 2 の判定を省略してよい。この例外は self 鍵にのみ適用し、他者鍵には適用しない
...
**運用上の対応:** メンバー除外時は `disclosed` エントリを確認し、除外メンバーがアクセス可能だった秘密値（DB パスワード、API キー、証明書等）を変更する。`rewrap --rotate-key` やメンバー更新だけで旧値の再有効化リスクは消えないため、外部システム側での失効・ローテーションと、Git 履歴からの巻き戻しを current HEAD に昇格させないためのリポジトリ運用を併用する。
...
graph TB
    ...
    subgraph public_key["PublicKey (workspace)"]
        PK_DOC["secretenv.public.key@5<br/>自己署名 + SSH attestation"]
    end
