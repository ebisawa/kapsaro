...
`wrap[].recipient_handle` は表示や監査補助のための識別情報であり、復号時の鍵選択や信頼判断の基準ではない。鍵世代の識別と束縛には `kid` を用いる。
...
| 行種別               | 内容                     | セキュリティ上の役割                        |
| ----------------- | ---------------------- | --------------------------------- |
| `:SECRETENV_KV 4` | 形式とバージョン               | 署名対象に含めることでダウングレード攻撃を防ぐ           |
| `:HEAD`           | `sid`、タイムスタンプなどのファイル文脈 | wrap とエントリ全体を同一ファイルに束縛する         |
| `:WRAP`           | MK の HPKE wrap と削除履歴   | 現在の受信者集合と鍵配送状態を表す                 |
| `KEY` 行           | 各エントリの暗号文             | `salt`、nonce、AEAD 情報を持つ自己完結的な暗号単位 |
...
復号時は、まず署名を検証し、§10 の信頼ポリシーを適用してから MK を unwrap する。その後、必要なエントリについてのみ CEK を導出し、`k`、`sid`、`p` を含む AAD で AEAD 復号する。

file-enc と同様に、署名検証は復号処理に先行する。

### 7.3 CEK 導出

- CEK 導出には HKDF-SHA256 を用い、文脈情報として `p = secretenv:kv:cek@4` と `sid` を含める。
- salt はエントリごとに独立して生成する。
- これにより、別ファイルからエントリをコピーしても同じ CEK にはならず、復号に失敗する。

### 7.4 エントリ AAD

- エントリ AAD には、dotenv KEY を表す `k`、ファイル識別子 `sid`、プロトコル識別子 `p = secretenv:kv:payload@4` を含める。
- `k` により、同一 kv-enc 内でのエントリ入れ替えを防ぐ。
- `sid` により、CEK 導出時の文脈とペイロードレイヤの文脈を整合させる。
- `salt` はすでに HKDF の salt 引数に使われており、受信者一覧は rewrap 時のペイロード固定性を保つため AAD に含めない。

### 7.5 HPKE wrap (kv)

- kv-enc でも HPKE wrap には `kid`、`sid`、`p = secretenv:kv:hpke-wrap@4` を含める。
...
受信者一覧（wrap 配列の `recipient_handle` 一覧）はペイロード AAD に含めない。
