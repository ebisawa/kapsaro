...
この信頼境界の中で、SSH 秘密鍵は 2 つの役割を担う（§7.2 で詳述）。1 つは attestation 署名者として SecretEnv 公開鍵と SSH 鍵を紐づける役割、もう 1 つはローカル鍵ストア上の PrivateKey ファイル（`private.json`）を暗号化している対称鍵を、復号のたびに SSH 署名から作り直すための署名提供者としての役割である。PrivateKey 素材そのものは、ローカル鍵ストア上の独立したファイルに保存されている。

したがって SecretEnv 鍵の復号が成立するのは、ローカル鍵ストア上の PrivateKey ファイル（`private.json`）に到達でき、かつそのファイルの内容から組み立てられる署名対象に対して SSH 署名を実行できる場合である。署名対象は PrivateKey ファイルの内容から導かれるため、ssh-agent への接続（agent forwarding を含む）や SSH 署名能力の提供そのものは、ローカル鍵ストアへの到達と組み合わさって初めて PrivateKey 保護に影響する。攻撃者が `private.json` と対応する SSH 秘密鍵または SSH 署名実行権限を得た場合、その鍵向けの過去暗号文は復号され得る。通常運用では SSH 鍵とローカル鍵ストアは同じ端末に共存するため、端末が侵害されるとこの条件が揃い得る。
...
- 第 1 層: ローカル鍵ストアが信頼境界の内側に置かれていること自体。OS / ファイルシステムのアクセス制御と鍵ディレクトリの所有権によって、`private.json` への到達が同じ利用者権限の範囲に限定される。通常運用ではこの層が主防御である
...
第 2 層の対称鍵を作り直す方式は 2 つある。PrivateKey そのものの形式や保存場所は両方式で共通であり、ローカルキーストア構造（§7.1.2）と暗号文フィールドを共有する。鍵導出経路と HKDF info を明示的に分離することで domain separation を保つ。
...
この方式で暗号化に使われる対称鍵（`enc_key`）は、SSH 鍵そのものとは別の、SSH 署名の出力から HKDF で導出される独立した鍵である。`enc_key` は都度の一時値として扱い、PrivateKey ファイルを読み解くたびに同じ手順で作り直す。再構成には SSH 鍵の署名能力に加え、対象 `private.json` の `protected` ヘッダ（`ikm_salt`、`hkdf_salt`、`kid` など）が必要である。両者が揃っていれば、同じ対称鍵を何度でも再導出できる。
...
このパイプラインが作り出すのは、PrivateKey ファイルの中身を暗号化・復号するための対称鍵である。PrivateKey 素材は、この対称鍵でファイルの中身を AEAD 復号した結果として得られる。
...
SSH ベースの PrivateKey 保護は、`enc_key` を SSH 署名能力から再導出する。SSH 署名能力は通常運用ではローカル鍵ストアと同じ端末上に置かれる。したがって本方式は、`private.json` だけが単体で漏洩した場合に対する追加の暗号化層を提供する。
