...
`kid` は §8 で HPKE seal/open の `info` / AAD、署名対象、および CEK 導出などに繰り返し現れる文脈束縛の中核識別子である。PublicKey 文書の意味のある内容（公開鍵素材、identity、`binding_claims`、`expires_at` 等）が変われば別の `kid` となり、古い wrap や署名文脈への流用が暗号学的に成立しなくなる。
