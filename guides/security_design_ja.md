...
SecretEnv はリポジトリを暗号学的な信頼の起点に置かず、暗号ファイル自身とローカル信頼領域に保護根拠を置く。保護対象は、共有される秘密値の機密性、暗号ファイルの改ざん検知、署名検証鍵の自己完結性、暗号コンポーネントの文脈束縛である。信頼境界は非対称である。ローカルキーストア・ローカル信頼ストア・SecretEnv 秘密鍵・SSH 署名能力はローカル信頼領域の内側に置き、リポジトリ上の現メンバー一覧・新規申請メンバー・暗号ファイルは改ざん可能な入力として扱う。

#### 暗号構成と検証順序

受信者へのコンテンツ鍵配布に HPKE Base mode、ペイロード・エントリ暗号化に XChaCha20-Poly1305、完全性と真正性確認に Ed25519 署名、決定的バイト列化に JCS を使う。システム全体の古典計算機上の暗号強度は 128-bit 相当である。

署名検証は自己完結する。file-enc と kv-enc の各ドキュメントは埋め込み `signer_pub` を持つ。検証器はワークスペース上の `members/active` やローカルキーストアから署名者鍵を探索せず、`signer_pub` 文書の構造・自己署名・SSH 署名による公開鍵の裏付けを検証したうえで、暗号ファイル本体の署名を検証する。

復号前の標準確認順序は、構造検証、`signer_pub` 検証、署名検証（`signature.mac` 含む）、信頼ポリシー判定、参照整合性検査、コンテンツ鍵の HPKE open、鍵保持証明検証、平文復号である。

#### 文脈束縛と形式ごとの差異

文脈束縛はコンポーネントの流用を防ぐ主要な設計要素である。`sid`（ファイル識別子）、`kid`（鍵ステートメント ID）、`k`（KV エントリ名）、`p`（プロトコル識別子）を、HPKE info、HPKE AAD、HKDF info、ペイロード AAD、署名対象の必要箇所に入れる。別ファイルのペイロード、別鍵世代の wrap、別エントリの暗号文、別プロトコル向けのデータの流用は、署名検証・HPKE open・AEAD 復号・参照整合性検査で失敗する。

file-enc と kv-enc は同じ信頼モデルを共有するが、鍵構造とローテーション時の挙動が異なる（詳細は §6、§7）。

#### 信頼ポリシー

暗号学的に正しい署名は、現在のワークスペースで受理してよいことを証明しない。SecretEnv は、埋め込み `signer_pub` による暗号学的検証、`members/active` による現在の認可判定、`known_keys` による鍵所有者承認、`recipient_sets` による書き込み経路の受信者集合承認、手動確認と GitHub online verify による本人性補助証拠を分離する。GitHub API は信頼の起点ではなく補助証拠である。

読み取り経路では、署名者の `kid` と現在の `members/active` に解決できる受信者の `kid` が `known_keys` で確認済み、または当該実行で手動承認される必要がある。書き込み経路では受信者集合を常に `members/active` から導出し、各 `kid` と受信者集合を保存前に確認する。期限切れ鍵は新規 wrap・署名生成には使えず、復号と操作対象 artifact の署名検証でのみ明示的な recovery 設定がある場合だけ許可する。`SECRETENV_STRICT_KEY_CHECKING=no` は明示指定した読み取り経路のローカル鍵承認確認を省略するだけで、署名検証・認可判定・書き込み経路には適用しない。

#### 残余リスクと監査観点

主な残余リスクは初回承認（TOFU）、過去開示の回収不能、受信者の長期秘密鍵漏洩によるwrap復号、リポジトリレベルのロールバック、ローカル信頼領域の侵害である。

監査上の優先確認項目は、復号前の署名検証と鍵保持証明検証、埋め込み `signer_pub` からのみの署名鍵解決、文脈束縛入力の維持、HPKE info と AAD の一致、PublicKey の自己署名と SSH 署名による公開鍵の裏付け、`members/active`・`known_keys`・`recipient_sets` の役割分離、期限切れ鍵 recovery の適用範囲、`SECRETENV_STRICT_KEY_CHECKING` の適用範囲、環境変数経由の鍵ロードを信頼できる CI 文脈に限ることである。
...
信頼境界は非対称に引く。ローカル秘密鍵・ローカルキーストア・ローカル信頼ストア・SSH 鍵を信頼の起点とし、ワークスペース上の `members/active/`・`members/incoming/`・`secrets/` は改ざん可能な入力として扱う。後者の真正性は暗号学的構造とリポジトリ運用統制の組合せで担保する（§2）。
...
本章は、機密性と完全性を支える標準暗号プリミティブの選定理由と、各プリミティブが標準仕様から継承する安全性・限界を整理する。選定基準は、IETF 標準への準拠、誤用の余地の構造的な最小化、SSH エコシステム（ssh-ed25519、SSHSIG）との親和性である。具体的な用途は §3.1 の一覧を参照。
...
XChaCha20-Poly1305 は 24-byte (192-bit) nonce を使用する。SecretEnv の設計では、同一の対称鍵で複数回の暗号化を行うケースが存在しない。MK（file-enc）・CEK（kv-enc エントリ）・enc_key（PrivateKey 保護）はそれぞれ暗号化ごとに一意に生成または導出されるため、nonce 衝突のリスクは構造的に排除されている。
...
本章は鍵素材を種類・役割・ライフサイクルの観点で整理する。`kid` は自己完結検証の起点であり文脈束縛の中核識別子である（§8）。各プロトコルでの消費詳細は §6、§7、§9 に譲る。
...
| SecretEnv 鍵ペア（X25519 + Ed25519） | `key new` で生成、`kid` 単位で世代管理     | 長期（`expires_at` まで active） | HPKE seal/open、Ed25519 署名・検証             | §5, §6, §7         |
...
SSH 鍵と SecretEnv 鍵ペアの関係と SSH 鍵の 2 つの役割（裏付けと PrivateKey 保護）は §9.1.1 で詳述する。MK・CEK の消費経路の詳細は §6・§7 に譲る。
...
- `mac` により、署名対象の artifact body、署名者 `kid`、HPKE open 後のコンテンツ鍵が対応することを後段で確認できるようにする
...
| `signer_pub` | `PublicKey@7` 文書        | 埋め込み署名者公開鍵                    | 署名検証鍵の唯一の出所。自己署名と SSH 署名による公開鍵の裏付けをこの文書上で検証する |
...
鍵保持証明の HMAC は、MAC 用 domain `secretenv:context:mac-domain:key-possession@2`、形式ごとの body bytes、`signature.kid` を length-framed した message に対して計算する。artifact 署名入力は別の domain `secretenv:context:sig-domain:artifact-signature@2` を使い、`signature.alg` / `signature.kid` から導出した `signature_header`、body bytes、`signature.mac` を length-framed する。file-enc / kv-enc ともに artifact MK から用途分離された MAC key を導出し、その派生鍵を HMAC key として使う。`signature.kid` は検証時に `signer_pub.protected.kid` と一致確認されるため、HMAC は内容鍵 possession を署名者の鍵ステートメントにも束縛する。`signer_pub` 全体の hash は含めない。
...
kv-enc では、`:SIG` 行を除く本文全体を、各実データ行を LF で終端して連結した canonical_bytes とし、`signature.alg` / `signature.kid` から導出した `signature_header`、canonical_bytes、`signature.mac` の ASCII bytes を署名用 domain の下で length-framed した値へ署名する（§7.1）。署名対象には `:SECRETENV_KV` バージョン行、`:HEAD` トークン行、`:WRAP` トークン行、すべての `KEY` 行、署名 metadata の `alg` / `kid`、鍵保持証明が含まれる。`:SIG` 行のうち署名値そのものは署名対象の外に置かれる。
...
- 層 A. `signer_pub` 文書の妥当性 — `signer_pub` が `PublicKey@7` として解釈可能であり、文書として改竄されていないことを確立する
...
`PublicKey@7` は、`protected` オブジェクトに対する自己署名を持つ。署名対象は公開鍵文書の `protected`（形式定義に従う）であり、フィールドの書き換えは自己署名検証失敗として検知される。
...
SSH 署名対象となる PublicKey attestation body は、`p = secretenv:context:sshsig-message:public-key:attestation@7`、`subject_handle`、`keys`、任意の `binding_claims`、任意の `created_at`、`expires_at` を含む JSON object を JCS 正規化した bytes である。`kid`、`attestation`、`signature` は含めない。SSHSIG signed_data は OpenSSH の `SSHSIG` 形式に従い、namespace を `secretenv-attestation`、reserved を空文字列、hash algorithm を `sha256`、message hash を `SHA256(JCS(attestation_body))` として構築する。
...
3. MK から file content key を artifact key schedule で導出し、ペイロードヘッダを JCS 正規化した値を AAD として XChaCha20-Poly1305 でファイル本体を暗号化する。
...
- wrap の文脈には、受信者鍵世代を示す `kid`、プロトコル識別子 `p = secretenv:context:hpke-info:file-enc:wrap@7`、ファイル識別子 `sid` を含める。
- HPKE の `info` と `AAD` には、同じ JCS 正規化済み文脈 bytes を用いる。これにより、鍵スケジュール側と AEAD 側で束縛入力が一致し、実装ずれが HPKE open 失敗として早期に表面化する。
- 受信者のメンバーハンドルは認可上重要だが、HPKE seal/open の暗号学的束縛は `kid` を基準に行う。

### 6.5 ペイロード暗号化

- ペイロードヘッダには `format = secretenv:format:file-enc:payload@7`、外側と同じ `sid`、AEAD 識別子 `xchacha20-poly1305` を含める。
...
kv-enc は `.env` 形式のキーバリューエントリを個別に暗号化する。二層鍵構造を採用しており、マスターキー（MK）を各受信者に HPKE seal で配送し、エントリごとの暗号鍵（CEK）は MK から HKDF で導出する。この設計により、個別エントリの部分復号やファイル全体を再暗号化しない効率的な更新が可能となる。
...
| 行種別               | 内容                     | セキュリティ上の役割                        |
| ----------------- | ---------------------- | --------------------------------- |
| `:SECRETENV_KV 9` | 形式とバージョン               | 署名対象に含めることでダウングレード攻撃を防ぐ           |
...
`:HEAD` は `sid`、VALUE AEAD、タイムスタンプを保持し、`:WRAP` は MK の wrap 配列と削除履歴を保持する。各 KEY 行の token は `nonce` と `ct` を含み、KEY は行頭トークンを使用する。署名対象は `:SIG` を除く本文全体である。canonical_bytes は各実データ行を LF 終端で連結した値として扱う。
...
暗号化時は、まず MK を生成して各受信者向けに HPKE seal を行い、artifact key schedule を 1 回構築する。その後で各エントリごとに nonce を生成し、`sid`、KEY 行の先頭トークン、nonce を含む HKDF で CEK を導出して AEAD 暗号化する。最後に `:SIG` を除く本文全体へ署名する。

復号時は、まず `signature.mac` を含む署名を検証し、§10 の信頼ポリシーを適用してから MK に対して HPKE open を行う。その後、HPKE open 後の MK から MAC key を導出して `signature.mac` を再計算し、鍵保持証明を検証する。必要なエントリについてのみ CEK を導出し、KEY 行の先頭トークン、`sid`、`p` を含む AAD で AEAD 復号する。

file-enc と同様に、署名検証は復号処理に先行する。

### 7.3 CEK 導出

- CEK 導出には HKDF-SHA256 を用い、文脈情報として `p = secretenv:context:hkdf-info:kv-enc:cek@9`、`sid`、KEY 行の先頭トークン、entry nonce を含める。
- artifact PRK は、MK と `sid` を含む JCS salt `secretenv:context:hkdf-salt:kv-enc@9` から 1 文書につき 1 回だけ抽出する。
- これにより、別ファイルや別 KEY へエントリをコピーしても同じ CEK にはならず、復号に失敗する。

### 7.4 エントリ AAD

- エントリ AAD には、KEY 行の先頭トークン、ファイル識別子 `sid`、プロトコル識別子 `p = secretenv:context:aad:kv-enc:entry-payload@9` を含める。
- KEY 行の先頭トークンにより、同一 kv-enc 内でのエントリ入れ替えを防ぐ。
- `sid` により、CEK 導出時の文脈とペイロードレイヤの文脈を整合させる。
- nonce は CEK 導出に含めるため、受信者一覧は rewrap 時のペイロード固定性を保つため AAD に含めない。

### 7.5 HPKE Seal/Open (kv)

- kv-enc でも HPKE seal/open には `kid`、`sid`、`p = secretenv:context:hpke-info:kv-enc:wrap@9` を含める。
...
これにより、HPKE seal/open の束縛入力 (`kid`, `p`, `sid`) が `info` と `AAD` で必ず一致する構造になる。将来の実装変更や別実装で片方の構築だけがずれた場合でも、その不整合は HPKE open failure として早期に表面化する。
...
SecretEnv の PrivateKey（KEM 秘密鍵 + 署名秘密鍵）は、ユーザーのローカルキーストア（`~/.config/secretenv/keys/`）に、鍵ごとの独立したファイル `private.json` として保存される。HPKE open や Ed25519 署名は、このファイルから取り出した PrivateKey 素材を使って実行される。
...
SSH 鍵はユーザー所有の既存認証鍵であり、SecretEnv 鍵ペア（アプリケーション専用）とは別物である（鍵階層は §4.1 参照）。PublicKey 側では裏付け、PrivateKey 側では `enc_key` 導出という 2 つの役割を担い、いずれも SSHSIG 名前空間で用途を分離する。
...
読み取り経路の目的は、攻撃者が変更できるリポジトリ上の暗号ファイルを、平文復号へ進めてよいか判断することである。SecretEnv は、構造検証、`signer_pub` 検証、`signature.mac` を含む署名検証、信頼判断、形式ごとの参照整合性検査、HPKE open 後の鍵保持証明検証を終えるまで平文復号へ進まない。
...
一方で、暗号ファイルに表示される受信者ハンドルが現在の active member file と矛盾しないことは、読み取り時の安全性に関わる整合性である。これは利用者が表示名や受信者集合を誤認する攻撃を検出するための境界であり、ローカル承認キャッシュの有無とは別に扱う。署名者が artifact recipient set に含まれることは読み取り経路の受理条件として扱わない。内容鍵 possession は、HPKE open 後の MK で `signature.mac` を検証することで確認する。
...
| 秘密情報の出力境界 | `--debug`、通常エラー、tracing、panic、test snapshot に MK、CEK、PrivateKey 保護用 IKM、raw SSH signature、平文、`SECRETENV_PRIVATE_KEY`、`SECRETENV_KEY_PASSWORD` の値を出さない | 診断経路やテスト成果物から秘密情報が漏れる |
| 定数時間比較 | MAC、AEAD tag、署名値、秘密由来 digest の比較に早期終了比較を使わない | 比較位置や一致 prefix から秘密由来情報が漏れる |
| ローカルキーストア permission | key load 時に permission を検査し、`private.json` が他者読み取り可能な場合は中断する | ローカル秘密鍵ファイルの漏洩を見逃す |
| 環境変数経由の鍵ロード               | 信頼できる CI 実行文脈でのみ利用し、fork PR、untrusted PR、`pull_request_target`、attacker-controlled checkout、untrusted runner では使用しない。鍵ロード時に self PublicKey をワークスペース上で探索しない                | 攻撃者制御下のチェックアウトや runner で秘密鍵を誤用し得る           |
...
KEM 秘密鍵、署名秘密鍵、MK / CEK、復号後平文、そして SSHSIG ベース PrivateKey 保護で IKM として扱う raw Ed25519 署名値は、型システムが許す範囲で使用後にゼロ化することが望ましい。少なくとも、長寿命バッファ、通常ログ、`--debug` 出力、tracing、panic、test snapshot に残さないことが重要である。`SECRETENV_PRIVATE_KEY` と `SECRETENV_KEY_PASSWORD` の値も SecretEnv 自身の資格情報として扱い、診断出力や子プロセス環境へ漏らさない。
...
| PrivateKey 保護 | PrivateKey 保護に使う SSH 鍵または同等の署名実行権限と `private.json` が同時に漏洩すると、保護対象の SecretEnv 秘密鍵も漏洩したものとして扱う |
...
| 配布ポリシー | SecretEnv は「誰がどの秘密を持つべきか」を定義する中央ポリシーを提供しない |
| 圧縮 | 暗号化前の圧縮は行わない |

### 13.2 鍵漏洩と前方秘匿性

HPKE Base mode はエフェメラル鍵により wrap 単位のエフェメラル鍵分離を提供する。ただし、受信者の長期秘密鍵が漏洩した場合、その受信者向けの既存 wrap はすべて HPKE open 可能になる。SecretEnv はシステム全体として強い意味での前方秘匿性を主張しない。
...
graph TB
    ...
    subgraph public_key["PublicKey (ワークスペース)"]
        PK_DOC["secretenv:format:public-key@7<br/>自己署名 + SSH 署名裏付け"]
    end
...
- fork PR、untrusted PR、`pull_request_target`、attacker-controlled checkout、untrusted runner では `SECRETENV_PRIVATE_KEY` と `SECRETENV_KEY_PASSWORD` を注入しない
- シークレット注入後に attacker-controlled な ref を checkout しない

ローカルキーストアとローカル信頼ストア（§9, §10.4, §13.5）

- ローカルキーストアとローカル信頼ストアのアクセス権を確認する。`<SECRETENV_HOME>/` 配下のディレクトリは `0700`、ファイルは `0600` にする
- `private.json` が他者読み取り可能な状態で運用しない。検出時は鍵ロードを中断し、permission を修復する
