...
- wrap の文脈には、受信者鍵世代を示す `kid`、プロトコル識別子 `p = secretenv:context:hpke-info:file-enc:wrap@5`、ファイル識別子 `sid` を含める。
...
- ペイロードヘッダには `format = secretenv:format:file-enc:payload@5`、外側と同じ `sid`、AEAD 識別子 `xchacha20-poly1305` を含める。
...
| 行種別               | 内容                     | セキュリティ上の役割                        |
| ----------------- | ---------------------- | --------------------------------- |
| `:SECRETENV_KV 6` | 形式とバージョン               | 署名対象に含めることでダウングレード攻撃を防ぐ           |
...
- CEK 導出には HKDF-SHA256 を用い、文脈情報として `p = secretenv:context:hkdf-info:kv-enc:cek@6` と `sid` を含める。
- salt はエントリごとに独立して生成する。
- これにより、別ファイルからエントリをコピーしても同じ CEK にはならず、復号に失敗する。

### 7.4 エントリ AAD

- エントリ AAD には、KEY 行の先頭トークン、ファイル識別子 `sid`、プロトコル識別子 `p = secretenv:context:aad:kv-enc:entry-payload@6` を含める。
...
- kv-enc でも HPKE wrap には `kid`、`sid`、`p = secretenv:context:hpke-info:kv-enc:wrap@6` を含める。
...
ただし、署名者が受信者集合に含まれることは artifact 上の参照整合性を確認する検査であり、既存 artifact の更新権限を暗号学的に証明する境界ではない。
