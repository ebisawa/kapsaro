...
監査上の優先確認項目は、復号前の署名検証、署名鍵解決元としての埋め込み `signer_pub` 固定、unwrap 後かつ平文復号前の鍵保持証明検証、文脈束縛入力の維持、HPKE info と AAD の一致、PublicKey の自己署名と SSH 署名による公開鍵の裏付け、`members/active`、`known_keys`、書き込み経路の `recipient_sets` の役割分離、`SECRETENV_STRICT_KEY_CHECKING` の適用範囲、環境変数経由の鍵ロードを信頼できる CI 文脈に限ることである。これらは実装ミスではなく、成立すると主張しているセキュリティ特性を直接支える構造的条件である。
...
限定例外として、非メンバー例外受理と `SECRETENV_STRICT_KEY_CHECKING=no` がある。前者は現メンバー一覧に存在しない署名者の暗号ファイルを対話的 / 一回限り / ドキュメント単位で受理する例外であり、署名者を現在のメンバーに戻さず、ローカル承認キャッシュも更新しない。後者は、利用者が明示指定した読み取り経路で鍵所有者承認確認のみを省略する設定であり、現メンバー一覧の確認、受信者ハンドルの整合性、暗号学的署名検証、鍵保持証明検証は省略しない。
