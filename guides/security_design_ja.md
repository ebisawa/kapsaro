...
| 行種別               | 内容                     | セキュリティ上の役割                        |
| ----------------- | ---------------------- | --------------------------------- |
| `:SECRETENV_KV 7` | 形式とバージョン               | 署名対象に含めることでダウングレード攻撃を防ぐ           |
...
- CEK 導出には HKDF-SHA256 を用い、文脈情報として `p = secretenv:context:hkdf-info:kv-enc:cek@7`、`sid`、KEY 行の先頭トークンを含める。
- salt はエントリごとに独立して生成する。
- これにより、別ファイルや別 KEY へエントリをコピーしても同じ CEK にはならず、復号に失敗する。

### 7.4 エントリ AAD

- エントリ AAD には、KEY 行の先頭トークン、ファイル識別子 `sid`、プロトコル識別子 `p = secretenv:context:aad:kv-enc:entry-payload@7` を含める。
...
- kv-enc でも HPKE wrap には `kid`、`sid`、`p = secretenv:context:hpke-info:kv-enc:wrap@7` を含める。
