...
本書は、Kapsaro のセキュリティ設計について、保護対象と前提条件を明確にするための文書である。Kapsaro が提示するセキュリティ主張とその成立条件、設計上の検証点、残余リスク、非目標を一貫した形で示すことを目的とする。

各節では、アルゴリズムやデータ構造の個別説明にとどまらず、どの設計判断がどのセキュリティ主張を支え、どこに運用前提や制約があるのかが読み取れるように記述する。

### 対象読者

想定する読者は主に次の 2 種類である。関心に応じて、該当するセクションから読み進めるとよい。
...
本章では、Kapsaro がどのような問題意識から設計され、何をセキュリティ主張として扱うのかを示す。まず設計の出発点を述べ、続いて保証するもの、運用前提に依存するもの、保証しないものを順に区別する。

以降の章では、ここで示す主張に対して、脅威モデル、信頼境界、暗号プリミティブ、署名検証、file-enc / kv-enc の具体構造、文脈束縛、信頼ポリシー、攻撃シナリオの順に根拠を示す。

### 1.1 設計の出発点

Kapsaro は、`.env`、証明書、API キーなどの秘密値を、チームが Git のレビューと履歴の中で扱えるようにするためのオフライン優先 CLI である。前提にある課題は、秘密値をチャットや手作業で配ると平文が残りやすく、誰がどの時点で読めたかを追いにくく、メンバーの追加や削除、CI 用アクセスの変更が運用から漏れやすいことである。
...
この課題に対する Kapsaro の答えは、Git を配布媒体として使いながら、暗号学的な信頼をリポジトリの状態そのものには置かない、というものである。共有される秘密値は受信者ごとの鍵に向けて暗号化し、暗号ファイルは署名と文脈束縛により検証可能にする。各利用者の秘密鍵、ローカルキーストア、ローカル信頼ストア、SSH 署名能力はローカル信頼領域に置き、ワークスペース上のメンバー情報や暗号ファイルは検証して扱う入力とみなす。
...
この設計の下で Kapsaro が保証する範囲は次のとおりである。ここでいう保証は、実装が §1.5 の不変条件を維持し、§1.3 の運用前提が満たされる範囲で成立する。
...
| 鍵一貫性              | 公開鍵文書の自己署名（strict 検証）         | 元の秘密鍵が漏洩していない                  | 新規の悪意ある鍵作成は防げない                      | §5.5             |
...
| 鍵ステートメント ID  | Kapsaro 鍵ペアと公開鍵文書の世代を識別する ID（実装上は `kid`、§4.4.1 参照）。どの鍵で署名、復号、鍵配送を行うかを区別するために使う                             |
| 現メンバー一覧     | ワークスペース上で現在のメンバー集合と受信者集合の認可基準として扱うデータ（実装上は `members/active`、§10 参照）                                             |
...
| 本人性補強       | 鍵がどの人物やアカウントに紐付くかの判断材料を増やす運用層                                                                                   |
...
本章では、まず攻撃者が操作できるとみなす入力を示し、次にその前提の下で必要になる運用上の前提を述べる。そのうえで、ローカル端末側に置く信頼領域とリポジトリ上で検証して扱う領域を区別し、最後に暗号学的検証、現在の認可、利用者承認、本人性補助証拠という各層を説明する。

### 2.1 攻撃者モデル

Kapsaro が暗号設計で対処する主な攻撃は、リポジトリ上のデータ、公開鍵の受け入れ、暗号ファイル内の部品、初回承認、ローカル承認履歴のいずれかを狙う。次の表では、どの境界を操作できるかという観点で攻撃者を分類する。リポジトリ運用統制やローカル信頼領域の保護を破る攻撃者は、§2.2 の運用前提を満たさないケースとして別途扱う。
...
Kapsaro の暗号構造だけで完結しない判断は、利用者または組織側の運用前提に依存する。ただし、運用前提を暗号学的な信頼の起点と混同してはならない。Kapsaro は、改ざん可能な入力を検証し、検証結果を運用上の承認と組み合わせて扱う。
...
リポジトリレベルのロールバック：Git を配布媒体として使う以上、リポジトリの正規利用者が過去のコミットから古い暗号ファイルを取得できること自体は避けられない。したがって、書き込み権限を持つ攻撃者または内部者が、過去に正当だった暗号ファイルを現在の HEAD に戻す変更をコミットするリポジトリレベルのロールバックは、Kapsaro の文脈束縛だけでは検知も防止もできない。文脈束縛が保証するのは暗号ファイルの整合性と文脈整合であり、Git 履歴に対する最新性や単調な進行ではないためである。
...
この種のリスクは、Kapsaro の暗号設計だけで完全には検知も防止もできない。Kapsaro は、保護ブランチ、必須レビュー、変更管理、デプロイ前確認などのリポジトリ運用により、過去版の暗号ファイルが現在の HEAD に昇格されないことを前提とする。
...
- ワークスペースの `secrets/` ディレクトリ：署名で検証する
...
Kapsaro 鍵を復号するには、ローカルキーストアで対象の `private.json` に到達できることと、その内容に基づく署名対象に対して SSH 署名を実行できることの両方が必要である。端末侵害や、`private.json` と SSH 署名能力の同時取得については、§9.4 の信頼仮定で扱う。
...
本章では、機密性と完全性を支える標準暗号プリミティブについて、その選定理由と、標準仕様から継承する安全性および限界を述べる。選定基準は、IETF 標準への準拠、誤用しにくい構造、SSH エコシステム（ssh-ed25519、SSHSIG）との親和性である。具体的な用途は §3.1 の一覧を参照。
...
| Ed25519 (PureEdDSA)        | —                                   | RFC 8032                     | 署名と検証                                                                          |
...
- Base mode では各 wrap で一時的なカプセル化を行う（wrap ごとのエフェメラル性の解釈と限界は §13.2 を併せて参照）。受信者の長期秘密鍵が漏洩すると当該受信者向けの既存 wrap が復号可能になる点も §13.2 を参照
...
| 代替案                     | 不採用理由                                                                                                |
| ----------------------- | ---------------------------------------------------------------------------------------------------- |
| RSA-OAEP                | 鍵サイズと暗号文オーバーヘッドが大きく、DHKEM + HKDF + AEAD の標準構成としての HPKE に比べて wrap 文脈（info/AAD）との一貫した結合を取りにくい |
...
- `salt` パラメータにより、同一 IKM と同一 info でも異なる鍵を導出可能
...
JSON オブジェクトを決定論的にバイト列へ変換し、署名対象、認証付き追加データ、HPKE info の入力が実装間で揺れないようにする（§8 全体）。
...
- file-enc：`protected` 全体および `payload.protected` の署名と AAD の構築（§5.2、§6.4、§6.5）
...
§3.3 で述べた AEAD の前提（同一鍵での nonce 一意性）を、Kapsaro は鍵素材のライフサイクル設計によって次のように満たす。
...
§3.7 では、各プリミティブが提供する機能的な保証と Kapsaro における依存関係を確認した。本節では、それらを数値化した古典コンピュータ上の推定セキュリティレベル（ビット相当）に焦点を当て、システム全体の「最弱環」がどこにあるかを示す。
...
本章では、鍵素材を種類、役割、ライフサイクルの観点からまとめる。`kid` は自己完結検証の起点であり、文脈束縛の中核となる識別子である（§8）。各プロトコルでの具体的な使われ方は §6、§7、§9 で述べる。

### 4.1 鍵の種類と関係


| 鍵カテゴリ                           | 所有と出所                           | 寿命                         | 主用途                                        | 関連節                |
| ------------------------------- | ------------------------------- | -------------------------- | ------------------------------------------ | ------------------ |
| SSH Ed25519 鍵                   | ユーザー所有（Kapsaro 外）             | 長期                         | 公開鍵の SSH 署名裏付け、PrivateKey 保護（`enc_key` 導出） | §5.6, §9.2         |
| Kapsaro 鍵ペア（X25519 + Ed25519） | `key new` で生成、`kid` 単位で世代管理     | 長期（`expires_at` まで active） | HPKE seal/open、Ed25519 署名と検証             | §5, §6, §7         |
| MK / CEK / enc_key              | 暗号化操作ごとに CSPRNG または HKDF で生成または導出 | 短期（当該操作またはセッション内）            | file-enc / kv-enc のペイロード、PrivateKey AEAD   | §6.3, §7.3, §9.2.1 |
...
SSH 鍵と Kapsaro 鍵ペアの関係、および SSH 鍵が担う 2 つの役割（裏付けと PrivateKey 保護）は §9.1.1 で詳述する。MK と CEK がどこで消費されるかは §6 と §7 で述べる。
...
| ---------------------------- | -------- | --------------- | ---------------------------- | ----------------------------------------------------------------------------------------- | ---------- |
| SSH Ed25519 秘密鍵              | 32 bytes | ユーザーが管理         | 公開鍵の SSH 署名裏付け、PrivateKey 保護 | 直接の束縛入力ではない（公開鍵の裏付けと §9.2 の AAD が鍵ステートメント ID `kid` と接続）                                    | N/A（OS 管理） |
...
- 表の「ゼロ化要否」列の MUST は、対称鍵素材と署名秘密鍵について設計上必須の方針（実装は使用後にゼロ化を試みる）を示す。プロセスメモリからの完全消去は §12.3 のとおり保証しない。

### 4.3 受信者の資格

暗号化と復号における「受信者になれるか」は、暗号学的な本人性の証明ではなく、現在のワークスペースで誰に復号を許可するかという認可の問題である。§2.4 で層として分離したとおり、`members/active/` はその認可の基準情報であり、§10.3 の書き込み経路では受信者集合を常に現メンバー一覧から導出する。

暗号化操作の受信者になれるのは `members/active/` に記載されたメンバーのみである。`members/incoming/` の候補は §2.1 のとおりリポジトリ上の非信頼データであり、active に昇格するまでは既存の暗号ファイルを復号できるメンバーに含まれない。したがって `rewrap` で incoming を active に昇格させ、wrap を更新するまで、当該メンバーは既存の秘密を復号できない。incoming の昇格は受信者追加に相当し、§7.8 のとおり file-enc と kv-enc のいずれもコンテンツ鍵（MK）を維持したまま wrap エントリを追加する。対照的に、メンバー除外（受信者削除）時には両形式で MK を再生成し、file-enc はペイロード全体を、kv-enc は全エントリを再暗号化する（§7.8）。いずれの場合も「メンバーとして認められた」ことと「鍵が誰のものかの本人性」は別層であり、後者は TOFU やオンライン検証など §2.4 層4 の運用に依存する。
...
- active：`expires_at` が到来していない有効な状態。新たな暗号化（wrap）や署名の生成、および復号と検証に利用できる。
- expired：`expires_at` を過ぎた状態。新たな暗号化（wrap）や署名の生成は拒否される。過去に正当に署名または暗号化されたデータの復号や、操作対象 artifact の署名検証は、明示的な復旧設定がある場合だけ警告付きで許可される。
- rotate：`rewrap --rotate-key` などにより、新しい鍵ペア（新しい `kid`）を生成して active な鍵を切り替える。古い鍵素材は、自然に `expires_at` を迎えるまで、または運用で破棄するまで、過去の暗号ファイルの復号と検証用にローカルキーストアに保持される。
...
expired 後も、当該鍵で過去に作成された暗号ファイルの暗号学的整合性や文脈束縛が自動的に失効するわけではない。期限は「これ以降の新規 wrap と新規署名を拒否する」ための運用境界であり、復号や操作対象 artifact の署名検証に期限切れ鍵を使う場合も、`--allow-expired-key`、`KAPSARO_ALLOW_EXPIRED_KEY=yes`、または `allow_expired_key="yes"` による明示的な復旧設定が必要である。漏洩後の過去暗号文への被害限定は §13.2 で述べるとおり `--rotate-key` 等の別措置の対象である。
...
鍵ローテーションは主に `rewrap` コマンドで行う。主な契機は、受信者集合の変更（追加または削除）、`expires_at` の接近に伴う新世代への移行、鍵漏洩の疑いに対する被害限定（§13.2）などである。

file-enc はファイル単位の MK、kv-enc はファイル単位の MK とエントリごとの CEK という二層構造（§7.2）である。受信者追加では両形式とも既存コンテンツ鍵を維持して wrap を追加する。受信者削除では file-enc は MK を再生成してペイロード全体を再暗号化し、kv-enc は MK を再生成して全エントリを再暗号化する（§7.7, §7.8）。`--rotate-key` もコンテンツ鍵（MK）自体を再生成しペイロード全体を再暗号化する。いずれもプロトコル上の詳細は §7.8 で扱う。

`--rotate-key` は、漏洩後に将来の暗号ファイルへの被害拡大を抑える措置であり、§13.2 のとおり過去に配布済みの暗号文の機密性を「取り戻す」機構ではない。新しい `kid` を active として運用しつつ、古い `kid` は expired またはローカル保持のまま、過去の暗号ファイルの検証と復号に必要な期間だけ残す、という時間軸の整理が §4.4 と整合する。

---

## 5. 署名と検証アーキテクチャ

本章では、`signature_v4` 形式と、埋め込み公開鍵の妥当性を支える検証連鎖（自己署名、SSH 署名による公開鍵の裏付け、オンライン検証）を説明する。
...
file-enc は単一の JSON オブジェクトに `protected` と `signature` を並置する（§6.1）。kv-enc は行ベース文書の末尾に `:SIG` トークンを置き、本文の決定的表現と `signature.mac` へ署名する（§7.1）。いずれの形式も、Ed25519 による改ざん検知、`signature_v4` による署名検証鍵の自己完結、そして body、署名者 `kid`、コンテンツ鍵の対応を鍵保持証明で確認するという、同じ設計判断に基づいている。
...
この構造により、一部のメタデータだけを署名で守るのではなく、文書として解釈される本文全体と鍵保持証明の整合性が一つの Ed25519 署名に束ねられる。エントリを追加または更新しても、最終行の `:SIG` を除く範囲が変わるため、署名は文書全体に追従する。file-enc と同様、署名トークンの署名値そのものを署名対象に含めないのは自己参照を避けるためである。
...
これが確立するのは §1.6 の鍵一貫性である。すなわち「この公開鍵文書を作成した主体が、対応する Kapsaro 署名秘密鍵を保持していた」ことの証拠に過ぎず、本人性（その鍵が特定の人物や組織に帰属すること）の暗号学的証明ではない。攻撃者は自分の鍵ペアで新規に有効な自己署名付き PublicKey を作れるため、新規鍵挿入に対する主防御は §2.4 層2〜4 および §10 の運用側にある。一方、既存の PublicKey のフィールド改ざんは、元の秘密鍵なしには自己署名を整合的に更新できないため、層1の重要な支柱になる。

Kapsaro の Ed25519 検証はすべて strict であり、小位数の検証鍵と小位数の署名点 `R` をいずれも拒否する。これは鍵一貫性にとって本質的な条件である。小位数の検証鍵には対応する秘密鍵保持者が存在せず、その鍵の下では1個の署名が任意のメッセージに対して検証を通る。この拒否がなければ、秘密鍵を一切持たない者が自己署名付き PublicKey を公開でき、本節が依拠する証拠は無内容になる。
...
`binding_claims.github_account` が存在する PublicKey について、オンライン検証は、公開鍵の裏付けに用いた SSH 公開鍵が、検証時点で当該 GitHub アカウントに登録されている SSH 公開鍵集合に含まれるかを確認する。これは §2.4 層4 で述べる本人性判断の補助証拠であり、§5.4 の暗号学的検証連鎖（署名の真正性と文書整合性）の代替にはならない。

§5 の位置づけでは、オンライン検証を「署名検証に追加で重ねるオプションの証拠」と捉える。検証対象は GitHub 上の現在の鍵集合であり、履歴証明や暗号学的失効ではないこと、GitHub / SSH 信頼基盤侵害時には表示や API 応答自体が信頼できなくなることの含意は、§2.4 層4 に集約する。
...
file-enc は JSON 形式の署名付きコンテナである。監査上は次の要素が重要になる。
...
3. file-enc 固有の参照整合性を検査する。ここではペイロード形式と AEAD 識別子を確認し、外側 `sid` とペイロード内 `sid` の一致を確認する。
...
Kapsaro は、署名検証、信頼ポリシー判定、形式ごとの参照整合性検査、鍵保持証明検証をすべて通過するまで、平文を出力する処理へ進まない。これを設計上の不変条件としている。

---

## 7. kv-enc プロトコル

kv-enc は `.env` 形式のキーバリューエントリを個別に暗号化する。二層鍵構造を採用しており、マスターキー（MK）を各受信者に HPKE seal で配送し、エントリごとの暗号鍵（CEK）は MK から HKDF で導出する。この設計により、個別エントリだけの部分復号や、ファイル全体を再暗号化しない効率的な更新ができる。

### 7.1 データ構造の概観

kv-enc は行ベースの署名付き文書である。監査上は次の構造が重要になる。
...
受信者削除時は、両形式ともコンテンツ鍵を再生成する。file-enc では、削除された受信者の wrap エントリを除去し削除履歴を記録したうえで、MK を再生成してペイロード全体を再暗号化する。これは現在版の artifact から削除済み受信者の復号能力を取り除くための境界である。一方で、受信者削除時点でコンテンツ自体は過去の正当な受信者に開示済みであるため、削除済みメンバーがすでに見た内容、記憶、コピー済み平文には影響しない。過去に開示されたファイル内容を失効させたい場合は、証明書や秘密値そのものを外部システム側で失効させ、再発行する必要がある。
...
§6 と §7 では file-enc / kv-enc の構造と暗号化フローを個別に確認した。本章では、両形式に共通する束縛点を横断的に示す。Kapsaro は `sid` / `kid` / `k` / `p` を複数の場所に意図的に含めることで、「何を暗号化したものか」「どの鍵世代のものか」を暗号学的に固定している。
...
- CEK 導出の info に `sid` と `k` を含めるため、HKDF の段階でファイル識別子とエントリ識別子が CEK に影響する
- エントリ AAD にも `sid` と `k` を含めるため、AEAD の段階でも両識別子が検証される

暗号学的には一方のみで十分に見える箇所があるが、AAD にも含めることで、次の効果が得られる。

1. 実装バグ耐性：誤った `sid` で CEK を導出しても AEAD 検証で失敗する
2. 将来の変更への安全弁：CEK 導出ロジック変更時の検知層になる
3. 誤配線検知：異なるファイルの `sid` を誤適用した場合に早期検出できる
...
このパイプラインの結果として得られる `enc_key` は、`private.json` の `encrypted.ct` の暗号化と復号に使う対称鍵である。AEAD 復号が成功すると、内部の Kapsaro 秘密鍵材料が得られる。
...
実装上のメモリ衛生やログ衛生の観点については、後述の「§12.3 秘密情報のメモリ上の扱い」で説明する。
...
多くの CI プラットフォームには「シークレット変数」の仕組みがあり、値は安全に保存され、実行時に環境変数としてジョブへ渡される。この保護スキームにより、Kapsaro 秘密鍵をポータブルなパスワード保護形式でエクスポートし、CI シークレット変数として登録して SSH インフラなしで使用できる。
...
- エクスポート時の固定パラメータ：m=65536 (64 MiB), t=3, p=4。RFC 9106 Section 4 の "second recommended" オプションに準拠する
...
- ワークフロー / ジョブ定義がメンテナー管理下にあり、攻撃者制御の PR から変更も起動もできない
...
§2.4 で導入した 4 層モデルのとおり、`signer_pub`（暗号学的検証）、`members/active`（現在の認可）、ローカル信頼ストアの `known_keys` / `recipient_sets`（承認キャッシュ）はそれぞれ別の問いに答えるために設計されている。本章では、読み取り経路、書き込み経路、限定例外のそれぞれについて、その適用条件を述べる。
...
本章の各表では、攻撃を具体的に記述するため、これまで概念として扱ってきた `sid` / `kid` / `k` / `p` などの文脈束縛識別子や、`signer_pub`、`members/active`、`known_keys` といった内部名をそのまま使用する。各識別子の正式な定義と束縛点一覧は §8 を、信頼ポリシー上の役割は §10 を参照されたい。
...
| 自己署名と公開鍵の裏付け | 攻撃者は自分の鍵で有効な自己署名と SSH 署名による公開鍵の裏付けを生成可能                                    |
...
自己署名は既存 PublicKey の改ざんを防ぐが、攻撃者が自分の鍵で正規の手順に従って新規 PublicKey を作成することは防げない。新規鍵挿入に対する主防御は TOFU に基づく手動確認とリポジトリ運用統制であり、初回受け入れや初めて遭遇する署名者ではリポジトリ外チャネルでの確認が望ましい。
...
| 期待される失敗点 | 破損や不整合は検知可能だが、整合的な置換やロールバックは完全には防げない                               |
...
これらのシナリオには共通の構造がある。文脈束縛（`sid`, `kid`, `k`, `p`）と Ed25519 署名が 2 つの独立した防御層を形成している。両方を同時に迂回するには、署名者の秘密鍵の漏洩か、束縛の削除や処理順序の変更といった実装上の欠陥が必要となる。§12 の検証項目はこのような欠陥の検出を目的としている。
...
| ローカル状態のアクセス権 | ローカル状態のアクセス権と所有者、および `<KAPSARO_HOME>` に至る経路の書き込み権を検査し、逸脱を警告として報告する。秘密鍵ファイルだけは例外とし、他の利用者が読める状態、または他の利用者が所有する状態であれば読み込みを拒否する | 承認キャッシュと設定の漏洩を見逃す |
...
パースは fail-closed であり、上限が定められている。次の上限は形式契約の一部であり、独立実装はこの値までの文書を受理し、これを超える文書を拒否する必要がある。
...
以下は実装ガードである。パース前に資源消費を抑えるためのもので、形式を変えずに引き上げられる。

| 実装ガード | 値 |
| --- | --- |
| JSON の入れ子深さ | 32 |
| JSON の要素数 | wrap 項目数の上限から導出 |
| JSON 文書の読み込みサイズ | 24 MiB |

JSON 要素数の予算を固定値ではなく導出値としているのは、wrap 項目数が上限に達した文書をパース可能に保つためである。この閾値を下回る固定値では、wrap 数が検査されるより前のパース前走査で適合文書が拒否され、公表した上限が到達不能になる。

監査では次の点を確認する。

- 異常に大きい入力や過剰なネストで計算量やメモリ消費を爆発させないこと
- base64url では無効文字、padding (`=`)、空白や改行を拒否すること
- 固定長フィールドやアルゴリズム識別子に対して、厳密な長さと形式の検証を行うこと
- duplicate JSON member name を、typed model への変換や JCS 正規化より前に拒否すること

### 12.3 秘密情報のメモリ上の扱い

KEM 秘密鍵、署名秘密鍵、MK / CEK、復号後平文、そして SSHSIG ベース PrivateKey 保護で IKM として扱う生の Ed25519 署名値は、型システムが許す範囲で使用後にゼロ化することが望ましい。少なくとも、長寿命バッファ、通常ログ、`--debug` 出力、tracing、panic、test snapshot には残さない。`KAPSARO_PRIVATE_KEY` と `KAPSARO_KEY_PASSWORD` の値も Kapsaro 自身の資格情報として扱い、診断出力や子プロセス環境へ漏らさない。
...
| 同時更新 | 複数ホストから同じ保存先への同時書き込みは対応しない |
...
したがって、メンバー除外後や `rewrap --rotate-key` 後であっても、旧秘密値自体が外部システムでまだ有効なら再露出や再利用のリスクが残る。

メンバー除外時は `disclosed` エントリを確認し、除外メンバーがアクセス可能だった秘密値（DB パスワード、API キー、証明書等）を変更する。`rewrap --rotate-key` やメンバー更新だけで旧値の再有効化リスクは消えないため、外部システム側での失効とローテーション、および Git 履歴からの巻き戻しを現在の HEAD に昇格させないためのリポジトリ運用を併用する。
...
Kapsaro はローカル信頼領域を前提とする。端末上に置くファイルの保護は OS のアクセス制御に委ねており、この委譲には、利用者および共有マシンの管理者がアクセス制御を正しく設定しているという前提が含まれる。したがって Kapsaro は、ローカル状態に実際に設定されたアクセス権を尊重し、その範囲で動作する。
...
推奨する設定は、ローカル状態を全体として所有者専用にすることである。Unix ではディレクトリを `0700`、ファイルを `0600` とし、各エントリは現在の実効ユーザーが所有する。Kapsaro が自ら作成するエントリにはこの値を設定する。

前提が満たされているかを利用者が確認できるよう、Kapsaro はローカル状態のアクセス権と所有者を検査し、逸脱を警告として報告する。全体を一度に確認する手段は `kapsaro doctor` である。これは確認のための補助であって強制ではなく、処理は端末に実際に設定されたアクセス権のまま続行する。検査できなかった範囲を正常として扱うことはしない。

ただし秘密鍵ファイルは例外とし、他の利用者が読める状態、または他の利用者が所有する状態であれば読み込みを拒否する。他の文書は漏れても署名と暗号によって真正性と機密性が保たれるのに対し、秘密鍵はそれ自体が保護の根拠であり、露出した時点でその根拠が失われるためである。この中断は権限が実際に緩んでいる場合に限り、権限を確認できなかった場合は従来どおり警告として続行する。

`/` から `<KAPSARO_HOME>` に至る経路についても、別の利用者が書き込めるディレクトリがないかを検査する。書き込めるディレクトリが経路上にあれば、ローカル状態の木ごと差し替えられるためである。一方で祖先の所有者は検査しない。誰が所有するかはマシンの管理者が決める構成そのものであり、実行時に Kapsaro が埋められる差ではないからである。`kapsaro doctor` は判断材料として祖先の所有者を情報として示すが、これは指摘ではない。ローカル状態のエントリ自身については所有者を検査するので、この非対称は意図したものである。

経路上の symlink は `<KAPSARO_HOME>` 自身も含めて許容する。ローカル状態を暗号化ボリューム上のディレクトリへ向ける構成が正当な用途だからである。追従を禁じる代わりに、Kapsaro はディレクトリを一度開き、以後は開いたディレクトリを通じて処理する。これにより、リンクや経路上の要素が後から差し替えられても、処理先が別の場所へ移ることはない。`<KAPSARO_HOME>` 自身が symlink である場合は、リンクが置かれている側と解決先の両方の祖先を検査する。

Git で共有する成果物、すなわち `members/` 配下の公開文書と暗号化済みファイルはこの制約の対象外である。
...
Kapsaro は「誰がどの秘密を持つべきか」を定義する中央ポリシーを提供しない。暗号設計の妥当性と、組織側の承認と配布のプロセスは分離して評価すべきである。
...
- `<KAPSARO_HOME>` 配下のアクセス権と所有者を確認する。ディレクトリは `0700`、ファイルは `0600` とし、各エントリは現在の実効ユーザーが所有する
- `/` から `<KAPSARO_HOME>` に至る経路に、別の利用者が書き込めるディレクトリがないことを確認する
- `kapsaro doctor` を実行し、報告されたパスを環境が許す範囲で推奨設定に合わせる。そのままにするパスについては、その保護が周囲のアクセス制御に委ねられている状態として扱う
