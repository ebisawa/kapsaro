...
**SecretEnv が保証しないもの:** 復号後の内部者悪用の防止、過去に開示された秘密の回収、強い前方秘匿性、TOFU（Trust On First Use）を超える本人性保証。これらは見落としではなく、明示的な非目標である。とくに、PrivateKey 保護に使う SSH 秘密鍵と対応するローカルキーストアが同時に侵害された場合、その SSH 鍵で保護されている SecretEnv 鍵世代に向けた過去暗号文は復号可能になる。通常運用では両者は同じ端末に共存するため、ローカル信頼境界が破られた場合の現実的な帰結として扱う。

**利用者の運用責任:** ローカル信頼境界の適切な管理（ローカル端末、ローカルキーストア、SSH 鍵 / SSH 署名入力を含む）、`members/active/` への PR 変更のレビュー、TOFU 承認時のリポジトリ外チャネルを通じた新メンバー検証、メンバー除外時の秘密値のローテーション。
...
この信頼境界の中で、SSH 秘密鍵は単なる認証資産にとどまらず、SecretEnv PrivateKey 復元の実効的な trust root の一部としても機能する（§7.2 で詳述）。ただし SSH 秘密鍵単体の漏洩だけで SecretEnv 鍵世代が直ちに復号されるわけではなく、ローカルキーストアへの到達性が別途必要になる。通常運用では SSH 鍵とローカルキーストアは同じ端末に共存するため、ローカル信頼境界が破られた場合は、その SSH 鍵で保護された SecretEnv 鍵世代に向けた過去暗号文は復号可能として扱う。

一方で、ssh-agent や SSH 署名能力へのアクセスそのものを独立の脅威として強調するべきではない。実際に意味を持つのは、ローカルキーストアと SecretEnv 実行権限まで含めてローカル信頼境界が破られた場合である。これは個別の SSH 機能の問題というより、信頼されたローカル環境そのものの侵害であり、SecretEnv の設計だけで防ぐ対象ではない。
...
SecretEnv の PrivateKey（KEM 秘密鍵 + 署名秘密鍵）は、ユーザーのローカルキーストアに暗号化保存される。ここで保護対象となるのは workspace 上の暗号文ではなく、ローカルキーストア内の SecretEnv 秘密鍵そのものであり、復号された SecretEnv 秘密鍵が HPKE unwrap や Ed25519 署名生成に使われる。

### 7.1.1 保護対象と二つの保護方式

SecretEnv は PrivateKey の暗号化保護に二つの方式を提供する。

- SSH ベース保護（§7.2）: ユーザーの既存 SSH Ed25519 鍵を使って保護する。通常の対話的利用向けで、SecretEnv 固有のパスワード管理を不要にする
- パスワードベース保護（§7.3）: Argon2id + HKDF によるパスワード保護で、SSH インフラが使えない CI/CD 環境向け

両方式はローカルキーストア構造（§7.1.3）を共有しつつ、鍵導出経路と HKDF info を明示的に分離することで domain separation を保つ。信頼仮定の詳細は §7.4 で扱う。

### 7.1.2 SSH 鍵と SecretEnv 鍵ペアの関係
...
SSH ベース保護は、既存 SSH 鍵の署名能力を復号能力の源泉として利用することで、パスワードレスな PrivateKey 保護を実現する。SSH 秘密鍵そのものを暗号化鍵にするのではなく、SSH 署名から HKDF で派生した対称鍵（`enc_key`）を AEAD 鍵として使う点が特徴である。
...
| 段階 | 入力 | 出力 | セキュリティ上の意味 |
| --- | --- | --- | --- |
| SSHSIG 署名 | 署名メッセージ（`secretenv:key-protection-ikm@5` と `ikm_salt`）、namespace `secretenv-key-protection`、hash algorithm `sha256` | Ed25519 raw signature bytes | SSH 鍵を使える主体だけが次段階へ進める |
| HKDF-SHA256 | raw signature bytes、salt = `hkdf_salt`、info = `secretenv:sshsig-private-key-enc@5:{kid}` | `enc_key` | 鍵世代ごとに domain separation された対称鍵を導出する |
| XChaCha20-Poly1305 | `enc_key`、AAD = `jcs(protected)` | `encrypted.ct` | PrivateKey 文書ヘッダ全体の改ざん検知を伴って秘密鍵材料を保護する |

署名は OpenSSH `PROTOCOL.sshsig` 形式に従い、namespace `secretenv-key-protection` は attestation 用 namespace (`secretenv-attestation`) と明確に分離される。`kid` は SSH 署名メッセージ本体ではなく後段の HKDF info に含めることで、同じ SSH 鍵から派生する各世代の `enc_key` を相互に独立させる。

AAD は `jcs(protected)` であり、`format`、`member_id`、`kid`、アルゴリズム識別子、fingerprint、各 salt、タイムスタンプ、有効期限を含む復号条件全体が改ざん検知の対象になる。`enc_key` は保存済みの固定鍵ではなく、暗号化時と復号時の双方で同じ SSH 署名能力から再導出される。
...
したがって SSH 鍵はローカルキーストア保護の認証手段であるだけでなく、事実上の復号能力の源泉でもある。逆に言えば、SSH 秘密鍵を直接窃取しなくても、対象 private.json の protected ヘッダを読み取り、同等の署名要求を実行できる主体は `enc_key` を導出できる。この含意は §7.4 信頼仮定でさらに詳しく扱う。
...
SecretEnv の PrivateKey 保護は既存の SSH 鍵を trust anchor として再利用することで、独自のパスワードやマスター鍵管理を不要にする。ただし実効的な保護強度は SSH 鍵単体ではなく、ローカル信頼境界全体によって決まる。

SSH ベースの保護では、次の 3 要素すべてにアクセスできる主体が `enc_key` を再導出し、PrivateKey を復号できる。

1. 対象 private.json の protected ヘッダ
2. `secretenv-key-protection` namespace での SSH 署名実行権限
3. `encrypted.ct`

したがって実効的な保護対象は SSH 鍵だけではなく、ローカルキーストアと当該 PrivateKey を使う実行権限まで含む。通常運用ではこれらが利用者のローカル信頼環境にまとめて置かれることを前提とし、この境界が破られた時点で PrivateKey 保護も失われたものとして扱う。これは SecretEnv が個別に緩和する対象ではない。

| エンティティ | 復号可能性 | 備考 |
| --- | --- | --- |
| ローカルユーザー（信頼されたローカル環境） | **可能** | 正規の使用 |
| ローカルマルウェア | **可能（想定外）** | ローカルキーストア読み取りや当該 PrivateKey を使う実行権限を得た時点で PrivateKey 保護は失われたものとして扱う |
| 乗っ取られたユーザーセッション | **可能（想定外）** | 利用者権限のまま SecretEnv 実行やローカルキーストア参照が可能になる場合 |
| ユーザー権限または管理者権限を得た別プロセス | **可能（想定外）** | 到達経路にかかわらずローカル信頼境界が破られたものとして扱う |
| ハードウェアトークン（FIDO2） | **不可能** | Ed25519-SK は非決定論的署名のため IKM 導出不可。§7.2.2 の決定論性チェックで検出される |

要点は、鍵ファイル・ssh-agent・agent forwarding といった個別の SSH 機能を切り出して脅威評価するのではなく、ローカル信頼境界そのものの侵害を唯一の失敗条件とみなすことである。署名能力への到達経路が鍵ファイル、ssh-agent、同等の実行権限のいずれであっても結論は変わらない。agent forwarding もそれ単独で SecretEnv 固有のリスクとして扱うべきではなく、forwarding 先に保護対象レコードや鍵材料への到達性を与える構成が問題であり、これはローカル信頼境界の崩壊として扱う。
...
- PrivateKey 保護に使う SSH 秘密鍵が漏洩し、かつ対応するローカルキーストアへの到達性も併せて失われた場合、その SSH 鍵で保護されているすべての SecretEnv 秘密鍵世代は実質的に漏洩したものとして扱う必要があり、それらの鍵世代に向けた過去暗号文はすべて復号可能になる
...
**運用上の対応:** 鍵漏洩の疑いがある場合は、対象ファイルに対して直ちに `rewrap --rotate-key` を実行する。`--rotate-key` は将来の暗号化のみを保護するため、基礎となる秘密値（DB パスワード、API キー等）も合わせてローテーションする。漏洩元が PrivateKey 保護に使う SSH 鍵である場合は、同じ SSH 鍵が複数世代の SecretEnv 鍵を保護し得るため、この措置だけでは不十分であり、別の SSH 鍵または別の保護方式への移行を併用する必要がある。
...
- ローカル信頼境界が破られた場合は SecretEnv の防御対象外であることを理解し、その前提で端末と鍵保存領域を保護する（§7.4）
