...
PrivateKey 保護は SSH 署名から IKM を導出する設計のため、`**secretenv-key-protection` namespace で SSH 署名を実行できる主体は、PrivateKey の暗号化鍵を導出・復号できる**。この同値性は設計上意図されたものだが、信頼境界を明確にするために以下を記載する。
...
環境変数経由の鍵ロードは `run` / `decrypt` / `get` / `list` に限る。鍵ロード時は `SECRETENV_PRIVATE_KEY` を `SECRETENV_KEY_PASSWORD` で復号し、exported PrivateKey 自体の妥当性のみを確認する。`list` は鍵ロードを伴わないメタデータ表示である。

このとき、署名者自身の PublicKey を workspace の `members/active/` から取得して照合してはならない。`members/active/` の不在や内容不一致は、環境変数経由の鍵ロード自体の失敗条件ではない。

ただし `run` / `decrypt` / `get` の実行時には、通常どおり workspace checkout を参照しうる。workspace は引き続き信頼境界外の入力であるため、環境変数経由で `SECRETENV_PRIVATE_KEY` を使用してよいのは、以下をすべて満たす trusted CI context に限られる:
...
代表例として、protected branch の post-merge workflow や protected tag 上の deploy job は許容される。PR 検証系 workflow に secrets を出して使う運用は想定しない。
...
kv-enc では、`:SIG` 行を除く文書内容を正規化した canonical bytes に対して署名する。これにより `:HEAD`、`:WRAP`、各 entry 行を含む文書全体が改ざん検知の対象になる。詳細な canonicalization 手順は PRD に委ねる。
