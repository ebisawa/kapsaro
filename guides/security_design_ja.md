...
復号前の標準確認順序は、構造検証、`signer_pub` 検証、`signature.mac` を含む署名検証、信頼ポリシー判定、形式ごとの参照整合性検査、コンテンツ鍵 unwrap、key-possession proof 検証、平文復号である。署名検証、信頼ポリシー判定、または key-possession proof 検証より前にペイロードやエントリの復号へ進む実装は、攻撃者制御の暗号ファイルを復号処理へ渡すため、設計上の不変条件に反する。
...
監査上の優先確認項目は、復号前の署名検証、署名鍵解決元としての埋め込み `signer_pub` 固定、unwrap 後かつ平文復号前の key-possession proof 検証、文脈束縛入力の維持、HPKE info と AAD の一致、PublicKey の自己署名と SSH 署名による公開鍵の裏付け、`members/active`、`known_keys`、書き込み経路の `recipient_sets` の役割分離、`SECRETENV_STRICT_KEY_CHECKING` の適用範囲、環境変数経由の鍵ロードを信頼できる CI 文脈に限ることである。これらは実装ミスではなく、成立すると主張しているセキュリティ特性を直接支える構造的条件である。
...
| 不変条件                                                     | 崩れる主張                                |
| -------------------------------------------------------- | ------------------------------------ |
| 署名検証と key-possession proof 検証の前に平文復号しない                         | 改ざん検知、信頼ポリシー適用、検証できない入力を受理しない処理     |
...
限定例外として、非メンバー例外受理と `SECRETENV_STRICT_KEY_CHECKING=no` がある。前者は現メンバー一覧に存在しない署名者の暗号ファイルを対話的 / 一回限り / ドキュメント単位で受理する例外であり、署名者を現在のメンバーに戻さず、ローカル承認キャッシュも更新しない。後者は、利用者が明示指定した読み取り経路で鍵所有者承認確認のみを省略する設定であり、現メンバー一覧の確認、受信者ハンドルの整合性、暗号学的署名検証、key-possession proof 検証は省略しない。
...
- `mac` により、署名対象の artifact body と unwrap 後のコンテンツ鍵が対応することを後段で確認できるようにする
...
| `mac`        | `hmac-sha256:<base64url>` | key-possession proof             | artifact body と unwrap 後の DEK / MK の対応を検証するための HMAC |
...
file-enc は単一の JSON オブジェクトに `protected` と `signature` を並置する（§6.1）。kv-enc は行ベース文書の末尾に `:SIG` トークンを置き、本文の決定的表現と `signature.mac` へ署名する（§7.1）。いずれも Ed25519 による改ざん検知、`signature_v4` による自己完結鍵源、key-possession proof による body とコンテンツ鍵の対応確認という同じ設計判断の上に乗る。

### 5.2 file-enc 署名

file-enc では、トップレベルの `protected` オブジェクト全体を JCS で正規化したバイト列 `jcs(protected)` に `signature.mac` の ASCII bytes を連結した値を署名対象とする。したがって `sid`、`wrap[]`、`removed_recipients`、`payload`（内側の `payload.protected` と `payload.encrypted` を含む）、`created_at` / `updated_at` など、`protected` に含まれるフィールドと key-possession proof は、外側の Ed25519 署名によって改ざん検知の対象になる。これは §6.1 の JSON レイアウトと整合する。
...
kv-enc では、`:SIG` 行を除く本文全体を、各実データ行を LF で終端して連結した canonical_bytes とし、そこに `signature.mac` の ASCII bytes を連結した値へ署名する（§7.1）。署名対象には `:SECRETENV_KV` バージョン行、`:HEAD` トークン行、`:WRAP` トークン行、すべての `KEY` 行、key-possession proof が含まれる。`:SIG` 行のうち署名値そのものは署名対象の外に置かれる。

この構造により、一部のメタデータだけを署名で守るのではなく、文書として解釈される本文全体と key-possession proof の整合性が一つの Ed25519 署名に束ねられる。エントリを追加・更新しても、最終行の `:SIG` を除く範囲が変わるため、署名は文書全体に追従する。file-enc と同様、署名トークンの署名値そのものを署名対象に含めないのは自己参照を避けるためである。
...
- 層 C. 暗号ファイル本体と key-possession proof の改ざん検知 — §5.1 で定義した署名対象バイト列に対し、`signer_pub` 内の署名用公開鍵で `sig` を検証する

§1.4 の不変条件「署名検証と key-possession proof 検証の前に平文復号しない」と整合するよう、unwrap は層 C の検証が成功した後に続けられる。unwrap 後は、得られた DEK / MK で `signature.mac` を再計算し、artifact body とコンテンツ鍵が対応することを確認してから平文復号へ進む。この key-possession proof は、内容鍵を保持していたことの暗号学的確認であり、署名者の人物本人性や artifact 更新権限の証明ではない。
...
| `signature`         | `signature_v4` 形式の署名 | `protected` 全体と key-possession proof の完全性を保護する          |
...
5. unwrap した DEK で `signature.mac` を再計算し、artifact body に対する key-possession proof を検証する。
6. `jcs(payload.protected)` を AAD として AEAD 復号する。
7. 途中のいずれかが失敗した場合は安全側で拒否する。

重要なのは、SecretEnv が署名検証・信頼ポリシー判定・形式ごとの参照整合性検査・key-possession proof 検証を通過する前に、平文を得る復号処理へ進まないことを設計上の不変条件としている点である。
...
復号時は、まず `signature.mac` を含む署名を検証し、§10 の信頼ポリシーを適用してから MK を unwrap する。その後、unwrap した MK で `signature.mac` を再計算して key-possession proof を検証し、必要なエントリについてのみ CEK を導出し、KEY 行の先頭トークン、`sid`、`p` を含む AAD で AEAD 復号する。
...
- `get` は、署名検証後に MK を復元し、key-possession proof を検証してから、対象 KEY の CEK だけを導出して当該エントリを復号する。
...
| file signature | `jcs(protected)` に `ascii(signature.mac)` を連結 | `sid`, `wrap[].kid`, ペイロード全体、key-possession proof | file-enc 署名検証 | file-enc コンテナ、wrap、ペイロード、proof の改ざん |
...
| kv signature | canonical_bytes に `ascii(signature.mac)` を連結 | `:HEAD`, `:WRAP`, KEY 行、key-possession proof | kv-enc 署名検証 | kv-enc 本文、wrap、エントリ、開示フラグ、proof の改ざん |
...
| 値                    | AAD に含めない理由                                                        | 保護手段                                                                                              |
| -------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
| 受信者一覧 / wrap 配列 | 受信者追加時にペイロードまたはエントリを固定したまま wrap を追加できるようにするため             | file-enc では `protected` 署名、kv-enc では canonical_bytes 署名。読み取り経路では key-possession proof 検証、書き込み経路では受信者集合承認も適用する |
...
読み取り経路の目的は、攻撃者が変更できるリポジトリ上の暗号ファイルを、平文復号へ進めてよいか判断することである。SecretEnv は、構造検証、`signer_pub` 検証、`signature.mac` を含む署名検証、信頼判断、形式ごとの参照整合性検査、unwrap 後の key-possession proof 検証を終えるまで平文復号へ進まない。
...
`SECRETENV_STRICT_KEY_CHECKING=no` は、利用者が明示指定した読み取り経路でローカル鍵所有者承認確認だけを緩和する。署名検証、`members/active` による現在の認可、受信者ハンドルの整合性、key-possession proof 検証は引き続き必要である。この設定は書き込み経路には適用せず、`known_keys` や `recipient_sets` を暗黙に更新する効果も持たない。CI などで使う場合も、その実行文脈自体を利用者が信頼できることが前提になる。
...
監査者は、構造検証 → `signer_pub` 検証 → `signature.mac` を含む署名検証 → 信頼ポリシー判定 → 参照整合性検査 → コンテンツ鍵 unwrap → key-possession proof 検証 → 平文復号を標準の確認順序として扱う。file-enc の外側 `sid` とペイロード内 `sid` の一致確認は、形式ごとの参照整合性検査に含まれる。


| 確認対象                      | 期待される実装                                                                  | 逸脱時のリスク                            |
| ------------------------- | ------------------------------------------------------------------------ | ---------------------------------- |
| 処理順序                      | 構造検証 → `signer_pub` 検証 → 署名検証 → 信頼ポリシー判定 → 参照整合性検査 → unwrap → key-possession proof 検証 → 平文復号 | 改ざんデータや現在有効な信頼状態の外にあるデータを先に復号してしまう |
...
| key-possession proof        | unwrap 後の DEK / MK で `signature.mac` を再計算し、平文復号前に一致を確認する                 | 署名済み body とコンテンツ鍵の対応が検証されない       |
...
| STRICT_KEY_CHECKING の適用範囲 | `SECRETENV_STRICT_KEY_CHECKING=no` は利用者が明示指定した読み取り経路のみに限定し、書き込み経路と key-possession proof 検証では無効にする | CI や日常運用で承認キャッシュが意図せず恒久無効化される      |
