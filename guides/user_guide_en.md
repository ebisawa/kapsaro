...
# You will be prompted to enter and confirm a password (minimum 8 UTF-8 bytes)
```

> Password strength: The 8-byte minimum is an implementation-enforced floor, not a recommendation. For offline brute-force resistance, use a random string that is at least 20 UTF-8 bytes long (20 or more characters for ASCII) or a passphrase with equivalent entropy. A CI secret variable generated by a password manager is ideal.
