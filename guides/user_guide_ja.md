...
`member verify --approve` は `members/active` だけを対象にします。offline verify と必要に応じた online verify を行ったうえで、`kid`、GitHub account の `id`、取得できた場合は `login`、SSH fingerprint を表示して確認します。承認された non-self の `kid` だけが local trust store の `known_keys` に記録されます。自分自身の active 鍵はローカルキーストアを trust anchor とするため、通常は `known_keys` に追加されません。
