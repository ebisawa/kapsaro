...
| `allow_non_member` | 非メンバー署名者の artifact を一回限りで対話受理する確認フローを有効化するか。値は `yes` または `no` | `no` | `--allow-non-member` | `SECRETENV_ALLOW_NON_MEMBER` |
...
| `SECRETENV_ALLOW_NON_MEMBER` | 非メンバー署名者の artifact を一回限りで対話受理する確認フローを有効化するか: `yes`, `no` | `no` |
...
- `SECRETENV_ALLOW_NON_MEMBER=yes` は `decrypt`, `get`, `list`, `rewrap` の対話実行時だけ、非メンバー署名者の一回限りの確認フローを有効にします。非対話実行や `run` では有効になりません。
