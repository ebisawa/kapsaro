...
| コマンド | 説明 |
|---------|------|
| `secretenv set [-n <name>] [-m <handle>] <KEY> <VALUE>` | エントリを追加・更新 |
| `secretenv set [-n <name>] [-m <handle>] <KEY> --stdin` | 標準入力から値を読み込んでセット |
| `secretenv get [-n <name>] [-m <handle>] <KEY>` | 特定キーの値を取得・表示 |
| `secretenv get [-n <name>] [-m <handle>] --all` | 全エントリを取得・表示 |
| `secretenv get [-n <name>] [--all] --with-key` | `KEY="VALUE"` 形式で出力 |
| `secretenv unset [-n <name>] [-m <handle>] <KEY> [--force]` | エントリを削除。非対話環境では `--force` が必要 |
| `secretenv list [-n <name>] [--json]` | キー名の一覧を表示（値は表示しない） |
| `secretenv import [-n <name>] [-m <handle>] <file> [--json]` | `.env` ファイルを一括インポート |
| `secretenv run [-n <name>] [-m <handle>] -- <command> [args...]` | 秘密情報を環境変数として注入してコマンドを実行 |
...
| `secretenv decrypt [-m <handle>] [--kid <kid>] <file> (--out <path> \| --stdout)` | ファイルを復号 |
| `secretenv decrypt [-m <handle>] [--kid <kid>] --stdin (--out <path> \| --stdout)` | 標準入力からファイル暗号化 JSON（file-enc）を読み込んで復号 |
...
| `secretenv member remove <member_handle> [--force]` | メンバーをワークスペースから削除。非対話環境では `--force` が必要 |
| `secretenv rewrap [-m <handle>] [--rotate-key] [--clear-disclosure-history] [--target <path>...] [--json]` | 承認待ちメンバーを有効化し、暗号ファイルの受信者情報を更新 |
