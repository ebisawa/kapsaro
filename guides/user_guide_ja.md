...
メンバーを識別する文字列です。メールアドレス風の値を使うことが多いですが、実際のメールアドレスである必要はありません。チーム内で一意であれば十分です。
...
この章は、CI で secret を **読む必要がある場合だけ** 参照してください。人間の開発者と同じように CI から鍵管理や `rewrap` まで行うことは想定していません。基本的には、開発者マシンで作成した CI 専用鍵を CI に渡し、CI では `get` / `run` / `decrypt` などの読み取り系コマンドだけを実行します。

CI 環境では、secretenv はローカルキーストアではなく環境変数から秘密鍵とパスワードを読み取ります。環境変数による鍵読み込みで保証されるのは読み取り系コマンドであり、`run` / `decrypt` / `get` / `list` が利用できます。

CI ランナーは通常一時的な環境であり、ローカル信頼ストア（`~/.config/secretenv/trust/`）を持ちません。そのため、他メンバーの署名が付いた secret を読む trusted CI ジョブでは、`SECRETENV_STRICT_KEY_CHECKING=no` を明示設定する必要があります。これは「自分の端末に保存した承認履歴がない」ことによる確認だけを省略する設定であり、現在のメンバー確認や署名検証そのものは引き続き行われます。

それでも、CI が checkout した workspace を無条件に信用してよいわけではありません。環境変数による鍵読み込みは、信頼できる workflow、信頼できる ref、信頼できる runner で実行される job に限定してください。
...
trusted CI context で必要なものは 3 つだけです。逆に言えば、これ以外のローカル設定や SSH 環境を CI に持ち込む必要はありません。
...
local trust store を持たない trusted CI で、他メンバー署名の読み取りコマンドを実行する場合は `SECRETENV_STRICT_KEY_CHECKING=no` をその job にだけ明示設定してください。これはローカル承認履歴の確認だけを省略する設定であり、現在のメンバー確認や署名検証は省略しません。また承認履歴を自動更新することもありません。
...
- **`SECRETENV_STRICT_KEY_CHECKING=no` の範囲**: local trust store を持てない CI のための例外です。書き込み系コマンドには効かず、ローカル承認履歴を自動更新することもありません。
...
| コマンド | 説明 |
|---------|------|
| `secretenv init [--member-id <id>]` | 新しい Workspace を bootstrap し、最初のメンバーを active に登録 |
| `secretenv join [--member-id <id>] [--force]` | 既存 Workspace に参加申請、または鍵ローテーション用の公開鍵を staging（incoming に登録） |
