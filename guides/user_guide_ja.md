...
10. [Workspace のヘルスチェック](#10-workspace-のヘルスチェック)
11. [メンバー管理](#11-メンバー管理)
...
- kv-enc のキー名
- 受信者一覧（recipient の `member_handle` / `kid`）
- 署名者の `kid`
- 作成日時・更新日時
- 開示履歴
...
GitHub と連携して online verify を使う運用では、`attestation.pub` がその GitHub アカウント の**現在の** SSH 公開鍵一覧に含まれているかも確認します。つまり GitHub から SSH 公開鍵を削除すると、その鍵に依存する今後の online verify を止められます。これは既存の attestation を消すものではありませんが、将来の承認や trust 更新を止めるための実務上の停止手段として使えます。
...
パスワードやトークンを shell history に残したくない場合は、値をコマンドライン引数に書かず、`--stdin` で入力します。
...
`run` は親プロセスの環境変数を継承します。ただし、親環境に含まれる `SECRETENV_*` は子プロセスへ渡しません。復号した secret の値は最後に適用されるため、同じ名前の環境変数があれば secret 側の値で上書きされます。
...
# stdin から file-enc JSON を読み込んで復号
...
## 10. Workspace のヘルスチェック

`secretenv doctor` は、メンバー追加、CI/CD 設定、リリース準備、trust や recipient の warning 調査の前に workspace の状態を確認するためのコマンドです。
...
このコマンドは読み取り専用で、次の項目を確認します。

- Workspace 構造と Git との対応
- active / incoming の member file、鍵の有効期限、重複 `kid`、GitHub binding と検証状態
- local keystore と active private key の利用可否
- active member に対する local trust store の承認状態
- `.secretenv/secrets/` 配下の暗号 artifact
- `SECRETENV_PRIVATE_KEY` が設定されている場合の CI 環境変数鍵 readiness

Artifact の確認では metadata、署名、recipient、disclosure history を検査し、secret payload は暗号化されたまま扱います。

出力は summary、next actions、findings、healthy areas、details に分かれます。`--verbose` を使うと check id と低レベル reason も表示します。FAIL がある場合だけ終了ステータスは 1 になります。WARN と SKIP は終了ステータス 0 のため、運用者は local troubleshooting の流れを止めずに内容を確認できます。

`secretenv doctor` は承認プロンプトを出しません。鍵の trust、recipient set の確認、`rewrap` が推奨された場合は、finding の内容を確認してから next-action に表示されたコマンドを実行してください。
...
`member list` の通常出力では、各メンバーの member handle と `kid` が表示されます。複数の鍵世代がある場合や、`rewrap` 前後の状態を確認するときに参照してください。
...
ローカル trust store は、自分が確認済みの公開鍵と write-path の artifact member set を自分の端末に記録しておく場所です。承認済みの鍵が入っていると、以後の操作で同じ鍵所有者確認を繰り返さずに済みます。確認済みの member set は、書き込みコマンドが暗号 artifact を保存する前の共有先確認に使われます。

`trust keys list` を実行すると、現在自分の端末に保存されている承認済み鍵を確認できます。`trust recipients list` では、確認済みの artifact member set を確認できます。local trust store は基本的に承認が蓄積されていく仕組みなので、記録が増えすぎたときや、もう使われていない鍵、あらためて確認し直したい鍵や member set を整理したいときに使います。

通常は `member verify --approve` や interactive な承認で自動的に記録されるため、日常的に手で操作する必要はありません。誤って承認した鍵を見直したいとき、古い承認を整理したいとき、鍵や artifact member set の確認を最初からやり直したいときに使ってください。
...
# 確認済み artifact member set を一覧表示
secretenv trust recipients list

# 特定の artifact member set の確認記録を削除
secretenv trust recipients remove <sid>

# 古い鍵承認をまとめて削除
secretenv trust keys purge --older-than 180d --force

# 古い member set 確認記録をまとめて削除
secretenv trust recipients purge --older-than 180d --force
```

`trust keys ...` と `trust recipients ...` が変更するのは自分の端末上の記録だけです。workspace の `members/active` や暗号ファイルの recipients は変更されません。つまり、これらのコマンドは「チームのメンバー構成を変える」のではなく、「自分が次回どこまで再確認を求められるか」を変える操作です。
...
`member remove` は削除前に、そのメンバーがまだ受信者として含まれている暗号ファイルを preview し、続けて `rewrap` が必要であることを警告します。preview 中に壊れた artifact や署名検証に失敗する artifact が見つかった場合は warning を表示して一覧から除外し、削除処理自体は継続します。非対話環境では `--force` がない限り削除しません。
...
| 状態 | 説明 |
|------|------|
| active | 暗号化・署名に使用される鍵。member handle につき 1 つ |
...
CI ランナーは通常一時的な環境であり、ローカル信頼ストア（`~/.config/secretenv/trust/`）を持ちません。そのため、他メンバーの署名が付いた秘密情報を読む信頼できる CI ジョブでは、`SECRETENV_STRICT_KEY_CHECKING=no` を明示設定する必要があります。これは「自分の端末に保存した承認履歴がない」ことによる read-path の鍵確認だけを省略する設定です。現在のメンバー確認、recipient label の整合性確認、signer と recipient set の整合性確認、署名検証そのものは引き続き行われます。
...
ローカル信頼ストアを持たない信頼できる CI で、他メンバー署名の読み取りコマンドを実行する場合は `SECRETENV_STRICT_KEY_CHECKING=no` をそのジョブにだけ明示設定してください。これは read-path の `known_keys` 確認を省略する設定であり、現在のメンバー確認、recipient label の整合性確認、signer と recipient set の整合性確認、署名検証は省略しません。また明示的な review や approval なしに承認履歴を更新することもありません。
...
# パスワードの入力と確認を求められます（UTF-8 エンコード後最低 8 bytes）
```

> パスワードの強度について: 8 bytes は実装上の下限値であり、推奨値ではありません。オフラインブルートフォースへの耐性を考慮すると、UTF-8 エンコード後 20 bytes 以上のランダム文字列（ASCII の場合は 20 文字以上、または同等のエントロピーを持つパスフレーズ）を強く推奨します。パスワードマネージャーで生成した CI シークレット変数を使用するのが理想的です。
...
CI ジョブでは、環境変数経由での鍵ロードに対応した秘密情報の読み取りコマンドのみ使用できます。member handle は秘密鍵から自動的に決定されます。
...
- **`SECRETENV_STRICT_KEY_CHECKING=no` の範囲**: 本章前半で説明したとおり、これはローカル信頼ストアを持てない CI のための例外です。書き込み系コマンドには効かず、明示的な review や approval なしにローカル承認履歴を更新することもありません。non-interactive な書き込みコマンドでは、出力 member set に review が必要な場合、保存前に失敗します。
...
鍵はデフォルトで生成から1年後に期限切れになります。[12章](#12-鍵管理とローテーション) のローテーション手順に従ってください。`secretenv key new` で新しい鍵を生成し、`secretenv join` で staging し、PR マージ後に `secretenv rewrap` を実行します。

### Q: 復号時に予期しない承認プロンプトが表示される

署名者の `kid` または artifact 内の active recipient `kid` が自分の端末で未確認の場合に発生します。`secretenv member verify --approve` を実行して、現在の active メンバーを確認・承認してください。読み取りコマンドで recipient kid が `members/active` に存在しないという warning が表示された場合、その artifact には古い recipient 情報が残っている可能性があります。書き込む前に `secretenv rewrap` を実行してください。
...
| `--json` | JSON 形式で出力 |
| `-q` / `--quiet` | 最小限の出力 |
| `-v` / `--verbose` | コマンド固有の詳細表示を出力 |
| `--debug` | 内部 debug trace log を出力 |
...
| `secretenv decrypt <file> (--out <path> \| --stdout)` | ファイルを復号 |
| `secretenv decrypt --stdin (--out <path> \| --stdout)` | stdin から file-enc JSON を読み込んで復号 |
...
| コマンド | 説明 |
|---------|------|
| `secretenv doctor [--workspace <path>] [--home <path>] [--member-handle <id>] [--verbose] [--debug]` | workspace 構造、members、local trust state、暗号 artifact、CI 環境変数鍵 readiness を読み取り専用で確認 |

### メンバー管理

| コマンド | 説明 |
|---------|------|
| `secretenv member list` | 全メンバーと各 `kid` を一覧表示 |
...
| コマンド | 説明 |
|---------|------|
| `secretenv trust keys list` | local trust store に保存されている承認済み鍵を一覧表示 |
| `secretenv trust keys remove <kid>` | local trust store から特定の鍵の承認記録を削除 |
| `secretenv trust keys purge --older-than <duration> [-f, --force]` | 指定期間より古い鍵承認記録を削除 |
| `secretenv trust recipients list` | local trust store に保存されている確認済み artifact member set を一覧表示 |
| `secretenv trust recipients remove <sid>` | 特定の artifact member set の確認記録を削除 |
| `secretenv trust recipients purge --older-than <duration> [-f, --force]` | 指定期間より古い artifact member set 確認記録を削除 |
...
設定キー: `member_handle`, `workspace`, `ssh_signing_method`（`auto` / `ssh-agent` / `ssh-keygen`）, `ssh_identity`, `ssh_keygen_command`, `ssh_add_command`, `github_user`
...
# デフォルトの workspace を設定（Git リポジトリ外から実行する場合など）
...
Workspace Root は `--workspace`、`SECRETENV_WORKSPACE`、設定ファイルの `workspace`、カレントディレクトリからの自動検出の順で解決されます。
...
| `workspace` | デフォルトの Workspace Root パス。チルダ展開（`~/...`）対応 | （なし。未設定時は自動検出） | `-w` / `--workspace` | `SECRETENV_WORKSPACE` |
...
設定ファイルが存在しない場合、エラーにならず環境変数やデフォルト値にフォールバックします。ファイルが存在するが構文エラーの場合はエラーになります。`config get` / `config set` / `config unset` / `config list` はグローバル設定ファイルを操作するコマンドであり、設定された workspace の存在確認は行いません。
...
- `SECRETENV_PRIVATE_KEY` と `SECRETENV_KEY_PASSWORD` は、ローカル keystore が利用できない CI/CD 環境で使用します。`SECRETENV_PRIVATE_KEY` を設定する場合、`SECRETENV_KEY_PASSWORD` も必須です。詳細は [13章](#13-cicd-連携) を参照してください。
