...
pub(crate) struct LocalStateDiagnostics {
    pub(crate) checks: Vec<DoctorCheck>,
}

pub(crate) fn diagnose_local_state(
    ...
    checks.extend(diagnose_member_keystore(&keystore_root, &owner));
    Ok(LocalStateDiagnostics { checks })
}

pub(crate) fn diagnose_trust_store(
    options: &CommonCommandOptions,
    member_handle: Option<&str>,
...
fn diagnose_member_keystore(keystore_root: &Path, member_handle: &str) -> Vec<DoctorCheck> {
