...
pub(crate) fn diagnose_members(
    workspace: &WorkspaceRoot,
    verbose: bool,
) -> Result<Vec<DoctorCheck>> {
    let mut checks = Vec::new();
    checks.extend(diagnose_active_members(&workspace.root_path, verbose)?);
    checks.extend(diagnose_incoming_members(&workspace.root_path, verbose)?);
    checks.push(check_kid_uniqueness(&workspace.root_path)?);
    Ok(checks)
}

fn diagnose_active_members(workspace_root: &Path, verbose: bool) -> Result<Vec<DoctorCheck>> {
...
fn diagnose_incoming_members(workspace_root: &Path, verbose: bool) -> Result<Vec<DoctorCheck>> {
