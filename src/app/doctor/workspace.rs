...
pub(crate) struct DoctorWorkspaceState {
...
pub(crate) fn diagnose_workspace(options: &CommonCommandOptions) -> Result<DoctorWorkspaceState> {
...
fn resolve_doctor_workspace(
    ...
    if let Ok(Some(path)) = resolve_workspace_from_config_base(Some(base_dir)) {
