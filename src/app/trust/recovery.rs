...
#[derive(Debug)]
pub(crate) struct TrustStoreResetPlan {
    pub path: PathBuf,
    pub warning_message: String,
}

#[derive(Debug)]
pub(crate) struct TrustStoreResetOutcome {
    pub path: PathBuf,
}

pub(crate) fn requires_trust_store_reset(error: &Error) -> bool {
    matches!(
        error,
...
pub(crate) fn prepare_trust_store_reset_plan(
...
pub(crate) fn execute_trust_store_reset(
    plan: &TrustStoreResetPlan,
) -> Result<TrustStoreResetOutcome> {
    if !plan.path.exists() {
