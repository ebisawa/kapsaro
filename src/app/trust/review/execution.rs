...
pub(crate) fn review_and_save_artifact_recipient_set<EmitWarnings, ConfirmRecipientSet>(
    execution: TrustExecutionContext<'_>,
...
pub(crate) struct GeneratedArtifactRecipientSetReview<'a> {
    pub(crate) trust_ctx: &'a TrustContext,
    pub(crate) signer_kid: &'a str,
    pub(crate) recipient_set: &'a ArtifactRecipientSet,
    pub(crate) capability: CommandCapability,
    pub(crate) context_label: &'a str,
}

pub(crate) fn review_generated_artifact_recipient_set<EmitWarnings, ConfirmRecipientSet>(
    execution: TrustExecutionContext<'_>,
...
pub(crate) fn review_artifact_recipient_set_trust<ConfirmRecipientSet>(
    outcome: &ArtifactRecipientTrustOutcome,
    context_label: &str,
        ...
        ArtifactRecipientTrustOutcome::NeedsManualApproval(review) => {
            if confirm(outcome, context_label)? {
                ...
                Err(crate::Error::InvalidOperation {
                    message: "Recipient set approval declined".to_string(),
                })
            }
        }
    }
