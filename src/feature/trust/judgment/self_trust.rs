...
use super::identity::{IntoMemberId, TrustIdentity};
...
impl SelfTrustSet {
    pub fn new<M, I>(member_id: M, sig_xs: I) -> Self
    where
        M: IntoMemberId,
        I: IntoIterator<Item = [u8; 32]>,
    {
        Self::try_new(member_id, sig_xs).expect("self trust inputs must be valid")
    }

    pub fn try_new<M, I>(member_id: M, sig_xs: I) -> Result<Self>
    where
        M: IntoMemberId,
        I: IntoIterator<Item = [u8; 32]>,
    {
        let mut set = Self {
            member_id: Some(member_id.into_member_id()?),
            sig_xs: Vec::new(),
            keystore_root: None,
        };
    ...
    where
        ...
        I: IntoIterator<Item = [u8; 32]>,
    {
        let mut set = Self::try_new(member_id, sig_xs)?;
    ...
    pub fn contains_identity(&self, identity: &TrustIdentity) -> Result<bool> {
        ...
        if identity.member_id_value() != member_id {
            return Ok(false);
        }
    ...
    fn load_keystore_identity(&self, identity: &TrustIdentity) -> Result<bool> {
        ...
        let public_key_path =
            get_public_key_file_path_from_root(keystore_root, member_id.as_str(), identity.kid());
        if !public_key_path.exists() {
            return Ok(false);
        }

        let public_key = load_public_key(keystore_root, member_id.as_str(), identity.kid())?;
        validate_keystore_identity(member_id, identity, &public_key)?;
...
fn validate_keystore_identity(
    ...
    if public_key.protected.member_id != member_id.as_str() {
        return Err(crate::Error::Config {
            message: format!(
                "Local self key member_id mismatch for kid '{}': expected '{}', got '{}'",
                identity.kid(),
                member_id,
