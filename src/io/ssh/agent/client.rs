...
impl AgentClient {
    ...
    fn list_identities(&mut self) -> Result<Vec<super::validation::AgentIdentity>> {
        self.save_packet(&build_request_identities())?;
    ...
    fn save_packet(&mut self, body: &[u8]) -> Result<()> {
        let len = u32::try_from(body.len()).map_err(|_| {
            crate::Error::from(SshError::build_operation_failed_error(
                "ssh-agent request exceeds maximum encodable size",
            ))
        })?;
        self.save_bytes(&len.to_be_bytes())?;
        self.save_bytes(body)
    }

    fn load_packet(&mut self) -> Result<Vec<u8>> {
        let mut len_buf = [0u8; 4];
        self.load_exact_bytes(&mut len_buf)?;
        ...
        self.load_exact_bytes(&mut body)?;
        Ok(body)
    }

    fn save_bytes(&mut self, bytes: &[u8]) -> Result<()> {
        self.socket.write_all(bytes).map_err(build_write_error)
    }

    fn load_exact_bytes(&mut self, bytes: &mut [u8]) -> Result<()> {
        self.socket.read_exact(bytes).map_err(build_read_error)
    }
}
...
fn build_read_error(error: std::io::Error) -> crate::Error {
    crate::Error::from(SshError::build_operation_failed_error_with_source(
