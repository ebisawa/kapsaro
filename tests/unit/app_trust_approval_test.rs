...
#[test]
fn test_commit_known_key_approvals_rejects_self_candidate() {
    ...
    let result = commit_known_key_approvals(&options, &execution, &[candidate]);
...
#[test]
fn test_commit_known_key_approvals_uses_execution_context_for_signing() {
    let home = setup_test_keystore_from_fixtures(ALICE_MEMBER_ID);
    let options = build_test_command_options(home.path(), None);
    let execution = build_test_execution_context(&home, ALICE_MEMBER_ID, None);
    let candidate = ApprovedKnownKey::from_review(BOB_MEMBER_ID, BOB_KID, None, None);

    commit_known_key_approvals(&options, &execution, &[candidate]).unwrap();
...
#[test]
fn test_commit_known_key_approvals_persists_verified_github_evidence() {
    let home = setup_test_keystore_from_fixtures(ALICE_MEMBER_ID);
    let options = build_test_command_options(home.path(), None);
    let execution = build_test_execution_context(&home, ALICE_MEMBER_ID, None);
    let verified_github =
...
#[test]
fn test_commit_known_key_approvals_does_not_persist_raw_github_claim_from_manual_review() {
    let home = setup_test_keystore_from_fixtures(ALICE_MEMBER_ID);
    let options = build_test_command_options(home.path(), None);
    let execution = build_test_execution_context(&home, ALICE_MEMBER_ID, None);
    ...
    commit_known_key_approvals(&options, &execution, &[approval]).unwrap();
...
#[test]
fn test_commit_known_key_approvals_persists_verified_github_from_trust_review_candidate() {
    let home = setup_test_keystore_from_fixtures(ALICE_MEMBER_ID);
    let options = build_test_command_options(home.path(), None);
    let execution = build_test_execution_context(&home, ALICE_MEMBER_ID, None);
    let verified_github =
