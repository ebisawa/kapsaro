...
use crate::app::doctor::ci::diagnose_ci_readiness;
...
fn test_doctor_ci_invalid_env_key_reports_fail_and_strict_warning() {
    let _guard = EnvGuard::new(&[
        ...
        "SECRETENV_KEY_PASSWORD",
        "SECRETENV_STRICT_KEY_CHECKING",
    ]);
    std::env::set_var("SECRETENV_PRIVATE_KEY", "not-base64url");
    std::env::set_var("SECRETENV_KEY_PASSWORD", "password");
    ...
    .common_options();
    let checks = diagnose_ci_readiness(&options);
...
fn test_doctor_ci_env_key_absent_reports_skip() {
    ...
    std::env::remove_var("SECRETENV_PRIVATE_KEY");
    std::env::remove_var("SECRETENV_KEY_PASSWORD");
    let home = TempDir::new().unwrap();
    let options = DoctorRequest {
        workspace: None,
        home: Some(home.path().to_path_buf()),
