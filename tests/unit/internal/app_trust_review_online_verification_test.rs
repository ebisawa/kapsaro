...
fn test_review_candidate_for_confirmation_requires_online_verification_for_known_key() {
    ...
    match error {
        Error::Verify { rule, message } => {
            assert_eq!(rule, "E_TRUST_ONLINE_VERIFY_REQUIRED");
...
fn test_verify_trust_candidate_online_missing_public_key_error() {
    ...
    match error {
        Error::Verify { rule, message } => {
            assert_eq!(rule, "E_TRUST_REVIEW_SOURCE_MISSING");
            assert!(
                message.contains("Missing public key required for online verification"),
                "unexpected: {message}"
            );
        }
        other => panic!("expected verify error, got {other:?}"),
...
fn test_verify_trust_candidate_online_rejects_member_handle_mismatch() {
    ...
    match error {
        Error::Verify { rule, message } => {
            assert_eq!(rule, "E_TRUST_ONLINE_VERIFY_MISMATCH");
            assert!(
                message.contains("did not match candidate 'alice@example.com'"),
                "unexpected: {message}"
            );
        }
        other => panic!("expected verify error, got {other:?}"),
