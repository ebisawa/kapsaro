...
async fn test_parse_github_user_response_non_success_error() {
    ...
    match error {
        Error::Verify { rule, message } => {
            ...
            assert!(
                message.contains("GitHub user not found for login 'alice'"),
                "unexpected: {message}"
            );
            assert!(message.contains("404 Not Found"), "unexpected: {message}");
        }
...
async fn test_parse_github_user_response_invalid_json_error() {
    ...
    match error {
        Error::Verify { rule, message } => {
            assert_eq!(rule, "V-GITHUB-API");
            assert!(
                message.contains("Failed to parse GitHub user response"),
                "unexpected: {message}"
            );
        }
        other => panic!("expected verify error, got {other:?}"),
...
async fn test_parse_github_keys_response_invalid_json_error() {
    ...
    match error {
        Error::Verify { rule, message } => {
            assert_eq!(rule, "V-GITHUB-API");
            assert!(
                message.contains("Failed to parse GitHub API response"),
                "unexpected: {message}"
            );
        }
        other => panic!("expected verify error, got {other:?}"),
...
async fn test_fetch_github_user_by_login_rejects_invalid_login_before_transport() {
    ...
    match error {
        Error::InvalidArgument { message } => {
            assert!(message.contains("GitHub login"), "unexpected: {message}");
        }
